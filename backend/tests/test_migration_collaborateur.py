...
def test_1_migration_salarie_to_collaborateur(supabase: Client) -> bool:
    ...
    try:
        ...
        if result.data:
            print_error(
                f"Trouvé {len(result.data)} occurrences de 'salarie' dans user_company_accesses"
            )
            all_passed = False
        else:
    ...
    try:
        result = (
            supabase.table("profiles").select("role").eq("role", "salarie").execute()
        )
        if result.data:
            print_error(
                f"Trouvé {len(result.data)} occurrences de 'salarie' dans profiles"
            )
            all_passed = False
        else:
    ...
    try:
        result = (
            ...
            .select("base_role")
            .eq("base_role", "salarie")
            .execute()
        )
        if result.data:
            print_error(
                f"Trouvé {len(result.data)} occurrences de 'salarie' dans role_templates"
            )
            all_passed = False
        else:
    ...
    try:
        result = (
            ...
            .eq("required_role", "salarie")
            .execute()
        )
        if result.data:
            print_error(
                f"Trouvé {len(result.data)} occurrences de 'salarie' dans permissions"
            )
            all_passed = False
        else:
    ...
    try:
        ...
        print_info(
            f"Occurrences de 'collaborateur' trouvées: {len(result.data) if result.data else 0}"
        )
    except Exception as e:
...
def test_2_collaborateur_rh_role(supabase: Client) -> bool:
    ...
    try:
        ...
        print_info(
            f"Occurrences de 'collaborateur_rh' trouvées: {len(result.data) if result.data else 0}"
        )
        print_success(
            "Le rôle 'collaborateur_rh' est accepté dans user_company_accesses"
        )
    except Exception as e:
    ...
    try:
        result = (
            supabase.table("role_templates")
            .select("base_role")
            .eq("base_role", "collaborateur_rh")
            .limit(1)
            .execute()
        )
        print_info(
            f"Templates avec base_role='collaborateur_rh': {len(result.data) if result.data else 0}"
        )
    ...
    try:
        result = (
            supabase.table("permissions")
            .select("required_role")
            .eq("required_role", "collaborateur_rh")
            .limit(1)
            .execute()
        )
        print_info(
            f"Permissions avec required_role='collaborateur_rh': {len(result.data) if result.data else 0}"
        )
        print_success(
            "Le required_role 'collaborateur_rh' est accepté dans permissions"
        )
    except Exception as e:
        ...
        try:
            ...
            if count > 0:
                for template in result.data[:3]:
                    print(
                        f"    - {template.get('name', 'N/A')} (ID: {template.get('id', 'N/A')})"
                    )
        except Exception as e:
...
def test_5_custom_roles_with_access_types(supabase: Client) -> bool:
    ...
    try:
        result = (
            supabase.table("role_templates")
            .select("id, name, base_role, job_title, is_system")
            .eq("is_system", False)
            .limit(20)
            .execute()
        )
        ...
        if result.data:
            ...
            print_warning(
                "Aucun template personnalisé trouvé (normal si base de données vide)"
            )
        ...
        if result.data:
            for template in result.data:
                ...
                perms_result = (
                    ...
                    .select("permissions(id, code, label, required_role)")
                ...
                print_info(
                    f"Template '{template_name}' (base_role={base_role}): {perm_count} permission(s)"
                )
                ...
                if perm_count > 0:
                    ...
                    if base_role == "rh" and rh_perms == 0:
                        print_warning(
                            f"Template '{template_name}' a base_role='rh' mais aucune permission RH"
                        )
                    elif base_role == "collaborateur_rh" and rh_perms == 0:
                        print_warning(
                            f"Template '{template_name}' a base_role='collaborateur_rh' mais aucune permission RH"
                        )

                    print(
                        f"    → Permissions RH: {rh_perms}, Permissions Collaborateur: {collab_perms}"
                    )
        else:
    ...
    try:
        ...
        if result.data:
            ...
            if "salarie" in role_counts:
                print_error(
                    f"Trouvé {role_counts['salarie']} occurrence(s) de 'salarie' (devrait être 0)"
                )
                all_passed = False
            else:
            ...
            if "collaborateur" in role_counts:
                print_success(
                    f"Rôle 'collaborateur' trouvé: {role_counts['collaborateur']} occurrence(s)"
                )
            else:
                print_warning(
                    "Aucune occurrence de 'collaborateur' trouvée (normal si base vide)"
                )

            # Vérifier que 'collaborateur_rh' peut exister
            if "collaborateur_rh" in role_counts:
                print_success(
                    f"Rôle 'collaborateur_rh' trouvé: {role_counts['collaborateur_rh']} occurrence(s)"
                )
            else:
                print_info(
                    "Aucune occurrence de 'collaborateur_rh' trouvée (normal si pas encore créé)"
                )
        else:
...
def test_8_constraints_check(supabase: Client) -> bool:
    ...
    try:
        result = (
            supabase.table("user_company_accesses").select("role").limit(100).execute()
        )
...
def main():
    ...
    tests = [
        (
            "Migration 'salarie' → 'collaborateur'",
            test_1_migration_salarie_to_collaborateur,
        ),
    ...
    for test_name, result in results:
        status = (
            f"{Colors.GREEN}✅ PASSÉ{Colors.RESET}"
            if result
            else f"{Colors.RED}❌ ÉCHOUÉ{Colors.RESET}"
        )
