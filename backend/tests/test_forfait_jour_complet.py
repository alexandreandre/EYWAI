...
class ContextePaieMock:
    ...
    def __init__(
        ...
        self.baremes = {"primes": [], "conventions_collectives": {}}
...
def generer_calendrier_mois(
    annee: int, mois: int, jours_travailles: List[int]
) -> List[Dict[str, Any]]:
    """
    ...
    for jour in range(1, nb_jours + 1):
        ...
        if jour_semaine >= 5:
            calendrier.append({"jour": jour, "type": "weekend", "heures_prevues": 0})
...
def generer_calendrier_reel(
    ...
    for jour in range(1, nb_jours + 1):
        ...
        # Weekends non travaillés
        if jour_semaine >= 5:
            calendrier.append({"jour": jour, "type": "weekend", "heures_faites": 0})
        elif jour in jours_travailles:
            calendrier.append(
                {
                    "jour": jour,
...
def calculer_periode_paie(annee: int, mois: int) -> tuple[date, date]:
    ...
    def trouver_jour_reference(
        target_annee: int, target_mois: int, jour_cible: int, occurrence: int
    ) -> date:
    ...
    date_reference_precedente = trouver_jour_reference(
        annee_precedente, mois_precedent, 4, -2
    )
    decalage_vers_dimanche_precedent = 6 - date_reference_precedente.weekday()
    date_fin_periode_precedente = date_reference_precedente + timedelta(
...
def test_analyseur_avec_periode_paie():
    ...
    absences_sans_periode = [
        e for e in evenements_sans_periode if "absence" in e.get("type", "")
    ]
    ...
    absences_avec_periode = [
        e for e in evenements_avec_periode if "absence" in e.get("type", "")
    ]
    ...
    if len(absences_avec_periode) > len(absences_sans_periode):
        ...
        print(
            f"   Les absences du mois précédent ({len(absences_avec_periode) - len(absences_sans_periode)} jours) sont maintenant comptées."
        )
    else:
        print(
...
def test_calcul_brut_avec_absences():
    ...
    print(
        f"   - Nombre de jours d'absence : {resultat_brut.get('nombre_jours_absence_injustifiee', 0)}"
    )
...
def test_normalisation_donnees():
    ...
    # Vérification
    toutes_normalisees = all(d["heures_prevues"] in [0, 1] for d in donnees_normalisees)
        ...
        if ev_jour:
            ...
            try:
                ...
                if ev_jour <= nb_jours_mois:
                    ...
                    print(
                        f"⚠️  Avertissement: Jour {ev_jour} invalide pour {ev_mois}/{ev_annee}, ignoré",
                        file=sys.stderr,
                    )
            except ValueError as e:
                print(
                    f"⚠️  Erreur lors de la création de la date pour l'événement {ev}: {e}",
                    file=sys.stderr,
                )
