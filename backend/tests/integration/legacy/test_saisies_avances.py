...
class TestSaisiesAvances:
    ...
    def test_calculate_seizable_amount(self):
        ...
        assert seizable <= net_salary / Decimal("20") * Decimal("19"), (
            "Ne doit pas dépasser 19/20 du salaire"
        )

        print(
            f"✓ Test quotité saisissable : {float(seizable)}€ pour un salaire net de {float(net_salary)}€"
        )
    ...
    def test_apply_priority_order(self):
        ...
        assert ordered[0]["type"] == "pension_alimentaire", (
            "La pension alimentaire doit être en premier"
        )
    ...
    def test_get_advances_to_repay_structure(self):
        ...
        assert callable(get_advances_to_repay), (
            "get_advances_to_repay doit être une fonction"
        )
        ...
        assert "employee_id" in params, (
            "La fonction doit avoir un paramètre employee_id"
        )
    ...
    def test_get_seizures_for_period_structure(self):
        ...
        assert callable(get_seizures_for_period), (
            "get_seizures_for_period doit être une fonction"
        )
    ...
    def test_enrich_payslip_with_seizures(self):
        ...
        payslip_data = {
            "net_a_payer": 1500.0,
            "salaire_brut": 2000.0,
            "synthese_net": {"net_imposable": 1400.0},
        }
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    mock_employee_table.select.return_value.eq.return_value.single.return_value.execute.return_value.data = {
                        "id": "emp-1"
                    }
                    mock_supabase.table.side_effect = lambda name: (
                        mock_employee_table if name == "employees" else mock_table
                    )

                    enriched = enrich_payslip(payslip_data.copy(), "emp-1", 2026, 2)

                    # Vérifier que les saisies sont présentes
                    assert "retenues_saisies" in enriched, (
                        "Le bulletin doit contenir la section retenues_saisies"
                    )
                    assert enriched["retenues_saisies"]["total_preleve"] > 0, (
                        "Le total prélevé doit être > 0"
                    )
                    assert len(enriched["retenues_saisies"]["saisies"]) > 0, (
                        "Il doit y avoir au moins une saisie"
                    )

                    # Vérifier que le net à payer a été réduit
                    assert enriched["net_a_payer"] < payslip_data["net_a_payer"], (
                        "Le net à payer doit être réduit"
                    )

                    print(
                        f"✓ Test enrichissement saisies : {enriched['retenues_saisies']['total_preleve']}€ déduit"
                    )
            ...
            with patch(
                ...
                with patch(
                    ...
                    enriched = enrich_payslip(
                        payslip_data.copy(), "emp-1", 2026, 2, payslip_id="payslip-1"
                    )

                    # Vérifier que les avances sont présentes
                    assert "remboursements_avances" in enriched, (
                        "Le bulletin doit contenir la section remboursements_avances"
                    )
                    assert enriched["remboursements_avances"]["total_rembourse"] > 0, (
                        "Le total remboursé doit être > 0"
                    )
                    assert len(enriched["remboursements_avances"]["avances"]) > 0, (
                        "Il doit y avoir au moins une avance"
                    )

                    print(
                        f"✓ Test enrichissement avances : {enriched['remboursements_avances']['total_rembourse']}€ remboursé"
                    )
