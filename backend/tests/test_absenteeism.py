...
employees_resp = supabase.table('employees').select('id, first_name, last_name').execute()
...
print(f"   Total jours ouvrés (pour {len(employees)} employés): {total_working_days * len(employees)}\n")

# 3. Récupérer les absences validées
print("3. Récupération des absences validées...")
absences_resp = supabase.table('absence_requests').select(
    'employee_id, type, selected_days, status'
