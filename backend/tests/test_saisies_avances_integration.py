    ...
    if seizures:
        ...
        for seizure in seizures:
            ...
            if calculation_mode == 'fixe':
                ...
                deduction = min(net_a_payer_estime * (Decimal(str(percentage)) / Decimal("100")), seizable_amount)
                print(f"    Pourcentage: {percentage}%")
            else:
                deduction = min(amount, seizable_amount) if amount > 0 else seizable_amount
...
def test_payslip_enrichment():
    ...
    payslips = supabase.table('payslips').select('id, employee_id, year, month, payslip_data').limit(1).execute()
        ...
        if 'retenues_saisies' in enriched_data:
            ...
            if saisies.get('saisies'):
                for saisie in saisies['saisies']:
                    print(f"  - {saisie.get('type', 'N/A')} : {saisie.get('montant', 0)}€")
        else:
        ...
        if 'remboursements_avances' in enriched_data:
            ...
            if avances.get('avances'):
                for avance in avances['avances']:
                    print(f"  - {avance.get('date_avance', 'N/A')} : {avance.get('montant', 0)}€")
        else:
...
def test_database_integration():
    ...
    advances_count = supabase.table('salary_advances').select('id', count='exact').execute()
    print(f"  ✓ {advances_count.count} avance(s) dans la base de données")
    
    seizures_count = supabase.table('salary_seizures').select('id', count='exact').execute()
    print(f"  ✓ {seizures_count.count} saisie(s) dans la base de données")
    
    repayments_count = supabase.table('salary_advance_repayments').select('id', count='exact').execute()
