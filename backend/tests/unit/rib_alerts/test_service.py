...
from app.modules.rib_alerts.application.service import (
    list_rib_alerts,
    mark_read,
    resolve,
)
...
class TestListRibAlertsService:
    ...
    def test_returns_result_from_get_rib_alerts(self):
        ...
        expected = RibAlertListResult(
            alerts=[{"id": "a1", "title": "IBAN modifié"}], total=1
        )
        with patch(
            "app.modules.rib_alerts.application.service.get_rib_alerts",
            return_value=expected,
        ) as mock_get:
            result = list_rib_alerts(
    ...
    def test_passes_filters_through(self):
        ...
        with patch(
            "app.modules.rib_alerts.application.service.get_rib_alerts",
            return_value=RibAlertListResult(alerts=[], total=0),
        ) as mock_get:
...
class TestMarkReadService:
    ...
    def test_returns_true_when_command_succeeds(self):
        ...
        with patch(
            "app.modules.rib_alerts.application.service.mark_rib_alert_read",
            return_value=True,
        ) as mock_cmd:
...
class TestResolveService:
    ...
    def test_returns_true_when_command_succeeds(self):
        ...
        with patch(
            "app.modules.rib_alerts.application.service.resolve_rib_alert",
            return_value=True,
        ) as mock_cmd:
            result = resolve(
                alert_id="alert-1",
