...
def reprise_fin_aout(monkeypatch):
    ...
    monkeypatch.setattr(
        queries, "get_cp_opening_reference_dates", lambda ids: {EMPLOYE: date(2026, 8, 31)},
        raising=False,
    )
...
def test_le_mois_de_la_reprise_elle_meme_est_produit(reprise_fin_aout, monkeypatch):
    """Août se termine le 31 : la date de reprise est atteinte, on calcule."""
    appels = []
    monkeypatch.setattr(
        queries.absence_repository, "list_validated_for_employees",
        lambda ids: appels.append(ids) or [],
    )
    queries.get_absence_balances_for_payslip(EMPLOYE, 2026, 8)
    assert appels, "le calcul aurait dû être lancé"


def test_sans_reprise_datee_le_comportement_ne_change_pas(monkeypatch):
    monkeypatch.setattr(queries, "_parse_hire_date", lambda _id: date(2022, 12, 1))
    monkeypatch.setattr(
    ...
    assert appels, "sans date de reprise, rien ne doit être bloqué"
