...
def core_signature(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if payload.get("id") != ITEM_ID_TO_VALIDATE:
        raise ValueError(
            f"ID '{ITEM_ID_TO_VALIDATE}' attendu, reçu '{payload.get('id')}'"
        )
...
def _eq_metropole(a: List[Dict[str, Any]], b: List[Dict[str, Any]]) -> bool:
    ...
    for x, y in zip(a, b):
        if x["periode_sejour"] != y["periode_sejour"]:
            return False
        if not (
            _eq_float(x["repas"], y["repas"])
            and _eq_float(x["logement_paris_banlieue"], y["logement_paris_banlieue"])
            and _eq_float(x["logement_province"], y["logement_province"])
        ):
            return False
    return True
...
def _eq_outre_mer(a: List[Dict[str, Any]], b: List[Dict[str, Any]]) -> bool:
    ...
    for x, y in zip(a, b):
        if x["periode_sejour"] != y["periode_sejour"]:
            return False
        if not (
            _eq_float(x["hebergement"], y["hebergement"])
            and _eq_float(x["repas"], y["repas"])
        ):
            return False
    return True
