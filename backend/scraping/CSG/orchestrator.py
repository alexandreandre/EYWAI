...
def core_signature(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(sal, dict):
        try:
            sal_norm = {
                "deductible": None
                if sal.get("deductible") is None
                else round(float(sal["deductible"]), 6),
                "non_deductible": None
                if sal.get("non_deductible") is None
                else round(float(sal["non_deductible"]), 6),
            }
        except (ValueError, TypeError):
            logging.warning(
                f"Valeurs salariales non numériques '{sal}' reçues de {payload.get('__script')}."
            )
...
def debug_mismatch(payloads: List[Dict[str, Any]], sigs: List[Dict[str, Any]]) -> None:
    ...
    line = " | ".join(
        f"{p.get('__script', '?')}=[{short(s)}]" for p, s in zip(payloads, sigs)
    )
    ...
    for p, s in zip(payloads, sigs):
        ...
        logging.warning(
            f"  - {script} -> {json.dumps(s['valeurs'], ensure_ascii=False)} | source={src}"
        )
...
def apply_patch_in_memory(
    ...
    patch_data = {
        ...
        "salarial": patch_core["valeurs"][
            "salarial"
        ],  # L'objet {"deductible": ..., "non_deductible": ...}
