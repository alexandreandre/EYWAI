...
SCRAPER_NAME = "vieillessesalarial"
...
def debug_success(payloads: List[Dict[str, Any]], sigs: List[Dict[str, Any]]) -> None:
    ...
    logging.info(
        f"Concordance des taux Vieillesse Salariale trouvée entre: {', '.join(scripts)}"
    )
...
def apply_patch_in_memory(
    ...
    patch_core: Dict[
        str, Any
    ],  # La sortie de core_signature {"plafonne": ..., "deplafonne": ...}
) -> Dict[str, Any]:
    """
    Met à jour le bloc 'cotisations' avec les patchs 'retraite_secu_plafond' et 'retraite_secu_deplafond' (taux salariaux).
    """
    logging.info(
        f"Application des patchs '{ITEM_ID_PLAFOND}' et '{ITEM_ID_DEPLAFOND}' (Salarial) en mémoire..."
    )
...
def update_config_in_supabase(
    ...
    """
    comment = (
        f"Mise à jour automatique: {ITEM_ID_PLAFOND} & {ITEM_ID_DEPLAFOND} (Salarial)"
    )
    ...
    if current_config_data == new_config_data:
        logging.info(
            "Les données Vieillesse Salariale sont inchangées. Mise à jour de 'last_checked_at'."
        )
        try:
    ...
    # --- SCÉNARIO 2 : DIFFÉRENT ---
        ...
        logging.warning(
            f"Différence détectée pour Vieillesse Salariale. Création de la version {current_version + 1}..."
        )
    ...
    try:
        ...
        for i in range(len(sigs) - 1):
            ...
            if not are_equal:
                all_equal = False
                debug_mismatch(
                    payloads[i]["__script"], payloads[i + 1]["__script"], details
                )
                break
