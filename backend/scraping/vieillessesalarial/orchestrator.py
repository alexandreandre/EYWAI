...
ITEM_ID_PLAFOND = "retraite_secu_plafond"
ITEM_ID_DEPLAFOND = "retraite_secu_deplafond"
...
SCRIPTS_TO_RUN: List[Tuple[str, str]] = [
    ("vieillessesalarial.py", os.path.join(os.path.dirname(__file__), "vieillessesalarial.py")),
    ("vieillessesalarial_LegiSocial.py", os.path.join(os.path.dirname(__file__), "vieillessesalarial_LegiSocial.py")),
    ("vieillessesalarial_AI.py", os.path.join(os.path.dirname(__file__), "vieillessesalarial_AI.py")),
]
...
def core_signature(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Extrait la section des données (les taux) pour la comparaison."""
...
def compare_floats(a: Optional[float], b: Optional[float], tol: float = 1e-6) -> bool:
    ...
    if a is None or b is None: 
        logging.warning(f"Comparaison avec None: a={a}, b={b}")
        return a is b # Doit être None si l'un est None
    try:
...
def debug_mismatch(script_a: str, script_b: str, details: str) -> None:
    ...
    logging.warning("DISCORDANCE entre les sources pour Vieillesse Salariale:")
...
def debug_success(payloads: List[Dict[str, Any]], sigs: List[Dict[str, Any]]) -> None:
    ...
    logging.info(f"Concordance des taux Vieillesse Salariale trouvée entre: {', '.join(scripts)}")
...
def apply_patch_in_memory(
    ...
    patch_core: Dict[str, Any] # La sortie de core_signature {"plafonne": ..., "deplafonne": ...}
) -> Dict[str, Any]:
    """
    Met à jour le bloc 'cotisations' avec les patchs 'retraite_secu_plafond' et 'retraite_secu_deplafond' (taux salariaux).
    """
    logging.info(f"Application des patchs '{ITEM_ID_PLAFOND}' et '{ITEM_ID_DEPLAFOND}' (Salarial) en mémoire...")
    ...
    patch_plafond = patch_core.get("plafonne")
    patch_deplafond = patch_core.get("deplafonne")

    if patch_plafond is None or patch_deplafond is None:
        raise ValueError("Patch core invalide, 'plafonne' ou 'deplafonne' manquant.")

    found_plafond = False
    found_deplafond = False

    for item in cotisations_list:
        if isinstance(item, dict):
            if item.get("id") == ITEM_ID_PLAFOND:
                item["salarial"] = patch_plafond  # <-- Mise à jour du taux salarial
                found_plafond = True
            elif item.get("id") == ITEM_ID_DEPLAFOND:
                item["salarial"] = patch_deplafond # <-- Mise à jour du taux salarial
                found_deplafond = True

    if not found_plafond:
        logging.warning(f"Item '{ITEM_ID_PLAFOND}' non trouvé. Ajout au bloc.")
        cotisations_list.append({
            "id": ITEM_ID_PLAFOND,
            "libelle": "Sécurité sociale Vieillesse plafonnée",
            "base": "brut_plafonne",
            "salarial": patch_plafond, # Nouveau taux
            "patronal": 0.0855 # Valeur par défaut
        })

    if not found_deplafond:
        logging.warning(f"Item '{ITEM_ID_DEPLAFOND}' non trouvé. Ajout au bloc.")
        cotisations_list.append({
            "id": ITEM_ID_DEPLAFOND,
            "libelle": "Sécurité sociale Vieillesse déplafonnée",
            "base": "brut",
            "salarial": patch_deplafond, # Nouveau taux
            "patronal": 0.0202 # Valeur par défaut
        })
...
def update_config_in_supabase(
    ...
    """
    comment = f"Mise à jour automatique: {ITEM_ID_PLAFOND} & {ITEM_ID_DEPLAFOND} (Salarial)"
    ...
    if current_config_data == new_config_data:
        logging.info(f"Les données Vieillesse Salariale sont inchangées. Mise à jour de 'last_checked_at'.")
        try:
    ...
    # --- SCÉNARIO 2 : DIFFÉRENT ---
        ...
        logging.warning(f"Différence détectée pour Vieillesse Salariale. Création de la version {current_version + 1}...")
...
def main() -> None:
    """Orchestre l'ensemble du processus de mise à jour des taux de vieillesse salariaux."""
    logging.info(f"--- DÉBUT Orchestrateur Vieillesse Salariale ---")
    ...
    try:
        ...
        logging.info(f"--- FIN Orchestrateur Vieillesse Salariale ---")
