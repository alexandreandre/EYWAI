...
def get_taux_vieillesse_salarial() -> dict | None:
    ...
    try:
        ...
        if not salarie_section_text:
            raise ValueError(
                "La section 'Taux de cotisations salarié' est introuvable."
            )
        ...
        if not m:
            raise ValueError(
                "Motif des taux d'assurance vieillesse salariale introuvable."
            )
        ...
        print(
            f"  - Taux vieillesse déplafonné (salarial) trouvé : {taux_deplafonne * 100:.2f}%",
            file=sys.stderr,
        )
        print(
            f"  - Taux vieillesse plafonné (salarial) trouvé   : {taux_plafonne * 100:.2f}%",
            file=sys.stderr,
        )
...
def main():
    ...
    if not taux_data:
        print(
            "ERREUR CRITIQUE: Le scraping des taux de vieillesse salariaux a échoué.",
            file=sys.stderr,
        )
        sys.exit(1)
