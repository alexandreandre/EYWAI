...
def scrape_cfp_rates() -> dict | None:
    ...
    try:
        ...
        anchor_p = soup.find(
            "p",
            string=re.compile(
                r"Le taux de la contribution à la formation professionnelle varie"
            ),
        )
        if not anchor_p:
            raise ValueError(
                "Paragraphe d'ancre introuvable sur la page. La structure a peut-être changé."
            )
        ...
        motif_moins_11 = re.search(
            r"([0-9,]+\s*\%)\s*si vous employez\s*moins de 11\s*salariés",
            page_text,
            re.IGNORECASE,
        )
        motif_11_et_plus = re.search(
            r"([0-9,]+\s*\%)\s*si vous employez\s*11\s*salariés et plus",
            page_text,
            re.IGNORECASE,
        )
        ...
        taux_11_et_plus = (
            parse_taux(motif_11_et_plus.group(1)) if motif_11_et_plus else None
        )
