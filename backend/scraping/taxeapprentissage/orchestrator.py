...
def equal_core(a: Dict[str, Any], b: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if not a:
        return False, "La signature de la source primaire est vide."
    ...
    if section_b is None:
        logging.warning(
            "Section 'part_principale' absente de la source secondaire. Comparaison partielle."
        )
    else:
        for rate_key in section_a:
            if rate_key not in section_b:
                return (
                    False,
                    f"Clé '{rate_key}' manquante dans 'part_principale' de la source secondaire.",
                )
            if not compare_floats(section_a.get(rate_key), section_b.get(rate_key)):
                return (
                    False,
                    f"Mismatch dans 'part_principale.{rate_key}': {section_a.get(rate_key)} != {section_b.get(rate_key)}",
                )
    ...
    if section_b is None:
        logging.warning(
            "Section 'solde' absente de la source secondaire. Comparaison partielle."
        )
    else:
        for rate_key in section_a:
            if rate_key not in section_b:
                return (
                    False,
                    f"Clé '{rate_key}' manquante dans 'solde' de la source secondaire.",
                )
            if not compare_floats(section_a.get(rate_key), section_b.get(rate_key)):
                return (
                    False,
                    f"Mismatch dans 'solde.{rate_key}': {section_a.get(rate_key)} != {section_b.get(rate_key)}",
                )
...
def debug_success(payloads: List[Dict[str, Any]], sigs: List[Dict[str, Any]]) -> None:
    ...
    logging.info(
        f"Concordance des taux Taxe Apprentissage trouvée entre: {', '.join(scripts)}"
    )
...
def apply_patch_in_memory(
    ...
    """
    logging.info(
        f"Application des patchs '{ITEM_ID_TO_PATCH_PRINCIPALE}' et '{ITEM_ID_TO_PATCH_SOLDE}' en mémoire..."
    )
    ...
    if not found_principale:
        logging.warning(
            f"Item '{ITEM_ID_TO_PATCH_PRINCIPALE}' non trouvé. Ajout au bloc."
        )
...
def update_config_in_supabase(
    ...
    if current_config_data == new_config_data:
        logging.info(
            "Les données Taxe Apprentissage sont inchangées. Mise à jour de 'last_checked_at'."
        )
        try:
    ...
    # --- SCÉNARIO 2 : DIFFÉRENT ---
        ...
        logging.warning(
            f"Différence détectée pour Taxe Apprentissage. Création de la version {current_version + 1}..."
        )
