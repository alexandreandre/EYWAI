...
def parse_taux(text: str) -> float | None:
    ...
    try:
        cleaned_text = (
            text.replace(",", ".").replace("%", "").replace("\xa0", "").strip()
        )
...
def scrape_taxe_apprentissage_rates() -> dict | None:
    ...
    try:
        ...
        h5_principale = soup.find(
            "h5", string=re.compile("Part principale de la taxe d’apprentissage")
        )
        ...
        print(
            f"  - Taux Part Principale (Métropole) : {taux_metropole_principale * 100:.2f}%",
            file=sys.stderr,
        )
        print(
            f"  - Taux Part Principale (Alsace-Moselle) : {taux_alsace_moselle_principale * 100:.2f}%",
            file=sys.stderr,
        )

        # --- 2. Extraction du solde ---
        h5_solde = soup.find(
            "h5", string=re.compile("Solde de la taxe d’apprentissage")
        )
        if not h5_solde:
        ...
        print(
            f"  - Taux Solde (Métropole) : {taux_metropole_solde * 100:.2f}%",
            file=sys.stderr,
        )
        print(
            f"  - Taux Solde (Alsace-Moselle) : {taux_alsace_moselle_solde * 100:.2f}%",
            file=sys.stderr,
        )

        if not all(
            [
                taux_metropole_principale,
                taux_alsace_moselle_principale,
                taux_metropole_solde is not None,
            ]
        ):
...
def main():
    ...
    if not rates_data:
        print(
            "ERREUR CRITIQUE: Le scraping de la taxe d'apprentissage a échoué.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    total_metropole = part_principale.get("taux_metropole", 0.0) + solde.get(
        "taux_metropole", 0.0
    )
    total_alsace_moselle = part_principale.get("taux_alsace_moselle", 0.0) + solde.get(
