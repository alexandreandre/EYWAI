...
def scrape_fnal_rates() -> dict[str, float | None]:
    ...
    try:
        ...
        pat_50_et_plus = re.compile(
            r"Fnal\s*\(.*50\s+salari[éê]s\s+et\s+plus", re.IGNORECASE
        )
        ...
        print(
            f"  - Taux FNAL (< 50 salariés) trouvé : {taux_moins_50}", file=sys.stderr
        )
        print(
            f"  - Taux FNAL (50+ salariés) trouvé : {taux_50_et_plus}", file=sys.stderr
        )
...
def main() -> None:
    ...
    if (
        rates.get("patronal_moins_50") is None
        or rates.get("patronal_50_et_plus") is None
    ):
        print(
            "ERREUR CRITIQUE: Un ou plusieurs taux FNAL n'ont pas pu être scrapés.",
            file=sys.stderr,
        )
        sys.exit(1)
