...
def scrape_fnal_rates_legisocial() -> dict[str, float | None]:
    ...
    try:
        ...
        pat_50_et_plus = re.compile(
            r"(50\s+salari[ée]s\s+et\s+plus|au\s+moins\s+50)", re.IGNORECASE
        )
        ...
        print(
            f"[DEBUG] Nombre total de titres trouvés: {len(all_titles)}",
            file=sys.stderr,
        )
        for i, title in enumerate(
            all_titles[:10]
        ):  # Limiter à 10 pour éviter trop de logs
            title_text = title.get_text(" ", strip=True)
            print(
                f"[DEBUG]   Titre {i + 1} ({title.name}): {title_text[:100]}",
                file=sys.stderr,
            )
        ...
        table_title = soup.find(
            lambda tag: tag.name in ["h2", "h3"] and searched_text in tag.get_text()
        )

        if not table_title:
            # Essayer des variantes
            print(
                f"[DEBUG] Titre exact '{searched_text}' non trouvé. Recherche de variantes...",
                file=sys.stderr,
            )
            variants = [
            ...
            for variant in variants:
                ...
                table_title = soup.find(
                    lambda tag: (
                        ...
                        and variant.lower() in tag.get_text().lower()
                    )
                )
                if table_title:
        ...
        if not table_title:
            print(
                "[DEBUG] ❌ AUCUN titre de table trouvé avec les critères de recherche",
                file=sys.stderr,
            )
            raise ValueError(
                f"Titre de la table des cotisations introuvable. Texte recherché: '{searched_text}'. "
                f"Aucun titre H1/H2/H3/H4 ne contient ce texte."
            )

        print(
            f"[DEBUG] ✓ Titre trouvé: '{table_title.get_text(' ', strip=True)[:100]}'",
            file=sys.stderr,
        )
        ...
        if not table:
            ...
            print(
                "[DEBUG] Aucune table trouvée après le titre. Recherche dans toutes les tables...",
                file=sys.stderr,
            )
            all_tables = soup.find_all("table")
            print(
                f"[DEBUG] Nombre total de tables trouvées dans la page: {len(all_tables)}",
                file=sys.stderr,
            )
            ...
            for i, t in enumerate(all_tables):
                ...
                if pat_row.search(table_text):
                    ...
                    print(
                        f"[DEBUG]   ✗ Table {i + 1} ne contient pas 'FNAL'",
                        file=sys.stderr,
                    )

        if not table:
            print(
                "[DEBUG] ❌ AUCUNE table contenant 'FNAL' trouvée dans la page",
                file=sys.stderr,
            )
            raise ValueError(
                "Table des cotisations introuvable après le titre. Aucune table dans la page ne contient 'FNAL'."
            )

        print(
            f"[DEBUG] ✓ Table trouvée ({len(table.find_all('tr'))} lignes)",
            file=sys.stderr,
        )
        ...
        for row_idx, tr in enumerate(table.find_all("tr")):
            ...
            rows_with_fnal += 1
            print(
                f"[DEBUG]   Ligne {row_idx + 1} contient 'FNAL': '{label[:100]}'",
                file=sys.stderr,
            )
            ...
            print(
                f"[DEBUG]     → Dernière cellule: '{last_cell_text[:100]}'",
                file=sys.stderr,
            )
            ...
            if rate is None:
                print(
                    "[DEBUG]     → ✗ Aucun pourcentage trouvé dans la dernière cellule",
                    file=sys.stderr,
                )
                continue

            print(
                f"[DEBUG]     → ✓ Taux extrait: {rate} ({rate * 100}%)", file=sys.stderr
            )
            ...
            if pat_moins_50.search(label):
                taux_moins_50 = rate
                print(
                    "[DEBUG]     → → Assigné à 'moins_50' (détection par texte)",
                    file=sys.stderr,
                )
            elif pat_50_et_plus.search(label):
                taux_50_et_plus = rate
                print(
                    "[DEBUG]     → → Assigné à '50_et_plus' (détection par texte)",
                    file=sys.stderr,
                )
            # Méthode 2: Identifier par la valeur du taux (fallback)
                ...
                taux_moins_50 = rate
                print(
                    f"[DEBUG]     → → Assigné à 'moins_50' (détection par valeur: {rate * 100}% ≈ {FNAL_MOINS_50_RATE * 100}%)",
                    file=sys.stderr,
                )
            elif abs(rate - FNAL_50_ET_PLUS_RATE) < 0.0001:  # Tolérance pour arrondis
                taux_50_et_plus = rate
                print(
                    f"[DEBUG]     → → Assigné à '50_et_plus' (détection par valeur: {rate * 100}% ≈ {FNAL_50_ET_PLUS_RATE * 100}%)",
                    file=sys.stderr,
                )
            else:
                print(
                    f"[DEBUG]     → → ⚠ Taux trouvé ({rate * 100}%) mais ne correspond à aucun pattern (moins_50/50_et_plus)",
                    file=sys.stderr,
                )
        ...
        print(
            f"[DEBUG]   Taux FNAL (< 50 salariés) trouvé : {taux_moins_50}",
            file=sys.stderr,
        )
        print(
            f"[DEBUG]   Taux FNAL (50+ salariés) trouvé : {taux_50_et_plus}",
            file=sys.stderr,
        )
        ...
        if taux_moins_50 is None or taux_50_et_plus is None:
            ...
            print(
                f"[DEBUG] ❌ ERREUR: Taux manquants après parsing: {', '.join(missing)}",
                file=sys.stderr,
            )
...
def main() -> None:
    ...
    if missing_rates:
        print(
            f"[ERROR] ERREUR CRITIQUE: Taux FNAL manquants: {', '.join(missing_rates)}",
            file=sys.stderr,
        )
