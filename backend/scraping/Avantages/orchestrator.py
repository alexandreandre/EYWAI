...
def run_script(label: str, path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if out:
            try:
                logging.warning(f"Le scraper {label} a échoué (code {e.returncode}) mais a produit un JSON.")
...
def normalize_bareme(lst: Any) -> List[Dict[str, float]]:
    ...
    for row in lst:
        ...
        vpp = to_float(row.get("valeur_par_piece") or row.get("valeur_par_piece_suppl_eur"))
        
        if v1 is None or vpp is None:
            logging.warning(f"Ligne de barème logement ignorée (données manquantes): {row}")
            continue
            
        out.append({
            "remuneration_max_eur": rmax if rmax is not None else 9_999_999.99, # Utilise un plafond 'infini' si non spécifié
            "valeur_1_piece_eur": v1,
            "valeur_par_piece_suppl_eur": vpp,
        })
    return sorted(out, key=lambda x: x['remuneration_max_eur']) # Toujours trier pour la comparaison
...
def payload_to_core(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(payload, dict) and payload.get("type") == "param_bundle":
            ...
            core["repas"] = to_float(mp.get("repas_valeur_forfaitaire_eur") or mp.get("repas"))
            core["titre"] = to_float(mp.get("titre_restaurant_exoneration_max_eur") or mp.get("titre_restaurant"))
            core["logement"] = normalize_bareme(mp.get("logement_bareme_forfaitaire") or mp.get("logement"))
        ...
        if not av:
             av = payload.get("entreprise", {}).get("parametres_paie", {}).get("avantages_en_nature", {})
        
        if av: # Si on a trouvé un bloc avantages_en_nature
            core["repas"] = to_float(av.get("repas_valeur_forfaitaire"))
            core["titre"] = to_float(av.get("titre_restaurant_exoneration_max_patronale"))
            core["logement"] = normalize_bareme(av.get("logement_bareme_forfaitaire"))
            # Pas de source ici, car c'est un fallback local
            return core
        
        logging.warning(f"Format de payload non reconnu pour {script_name}. Tentative d'extraction échouée.")
            
    except Exception as e:
        logging.error(f"Erreur lors de la normalisation du payload pour {script_name}: {e}. Payload: {str(payload)[:200]}...")
...
def cores_equal(a: Dict[str, Any], b: Dict[str, Any], tol: float = 1e-6) -> bool:
    ...
    if not (compare_floats(a["repas"], b["repas"], tol) and compare_floats(a["titre"], b["titre"], tol)):
        return False
    ...
    for r1, r2 in zip(la, lb):
        if not (compare_floats(r1["remuneration_max_eur"], r2["remuneration_max_eur"], tol) and
                compare_floats(r1["valeur_1_piece_eur"], r2["valeur_1_piece_eur"], tol) and
                compare_floats(r1["valeur_par_piece_suppl_eur"], r2["valeur_par_piece_suppl_eur"], tol)):
            return False
...
def merge_sources(payloads: List[Dict[str, Any]], cores: List[Dict[str, Any]]) -> List[str]:
...
def main() -> None:
    ...
    try:
        ...
        for label, path in SCRIPTS_TO_RUN:
            ...
            if not payload:
                logging.warning(f"Le script {label} n'a retourné aucun payload. Ignoré.")
                continue
        ...
        if all(not p for p in payloads):
            logging.critical("Tous les scrapers ont échoué ou n'ont retourné aucune donnée. Arrêt.")
            sys.exit(1)
