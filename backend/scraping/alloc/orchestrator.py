...
def core_signature(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logging.warning(f"Valeurs non numériques (plein='{plein}', reduit='{reduit}') reçues de {payload.get('__script')}.")
...
def debug_comparison(
    ...
    for p, s in zip(payloads, sigs):
        ...
        logging.info(f"  - {label:>20} -> plein={plein} | réduit={reduit} | source={src_str}")
    ...
    try:
        ...
        for i, sig in enumerate(sigs):
            if not validate_sig(sig):
                logging.error(f"Valeurs manquantes dans {labels[i]}: {json.dumps(sig, ensure_ascii=False)}")
                sys.exit(2)
