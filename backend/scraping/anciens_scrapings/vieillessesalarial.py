...
def get_taux_vieillesse_salarial() -> dict | None:
    ...
    try:
        ...
        if not salarie_section_text:
            raise ValueError(
                "La section 'Taux de cotisations salarié' est introuvable dans le HTML."
            )
        ...
        print(
            f" Taux vieillesse déplafonné (salarial) trouvé : {taux_deplafond * 100:.2f}%"
        )
        print(
            f" Taux vieillesse plafonné (salarial) trouvé   : {taux_plafond * 100:.2f}%"
        )
