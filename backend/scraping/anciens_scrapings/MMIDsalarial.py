...
def ajuster_taux_salarial_maladie():
    ...
    try:
        ...
        if is_alsace_moselle:
            print(
                f"Régime Alsace-Moselle détecté dans '{FICHIER_CONTRAT}'. Tentative de scraping..."
            )
        ...
        if taux_actuel == taux_correct:
            print(
                f"Le taux salarial maladie dans '{FICHIER_TAUX}' est déjà correct ({taux_correct}). Aucune modification nécessaire."
            )
            return
        ...
        print(
            f"ERREUR : La structure d'un fichier JSON est incorrecte. La clé {e} est manquante."
        )
    except Exception as e:
