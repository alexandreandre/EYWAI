...
def scrape_grand_deplacement(soup) -> dict:
    ...
    print(
        f"  - Barème Grand Déplacement trouvé (Métropole: {len(data['metropole'])}, OM1: {len(data['outre_mer_groupe1'])}, OM2: {len(data['outre_mer_groupe2'])})."
    )
    return data
...
def get_and_update_baremes():
    ...
    if not response:
        raise ConnectionError(
            "Impossible de récupérer la page web après plusieurs tentatives."
        )
    ...
    if not all(
        [
            repas_data,
            petit_deplacement_data,
            grand_deplacement_data,
            mutation_data,
            mobilite_data,
            teletravail_data,
        ]
