        ...
        if len(cells) == 2:
            ...
            if "hébergement provisoire" in libelle:
                data["hebergement_provisoire"]["montant_par_jour"] = (
                    parse_valeur_numerique(valeur_cell.get_text())
                )
            elif "installation dans le nouveau logement" in libelle:
                data["hebergement_definitif"]["frais_installation"] = (
                    parse_valeur_numerique(valeur_cell.get_text())
                )
            elif "majoré de par enfant" in libelle:
                # Cette cellule contient deux valeurs dans des balises <p> séparées
                valeurs = [
                    parse_valeur_numerique(p.get_text())
                    for p in valeur_cell.find_all("p")
                ]
                if len(valeurs) == 2:
...
def get_and_update_baremes_legisocial():
    try:
        ...
        print(
            "Note : Les barèmes 'petit déplacement' et 'télétravail' ne sont pas sur cette page et n'ont pas été mis à jour."
        )
