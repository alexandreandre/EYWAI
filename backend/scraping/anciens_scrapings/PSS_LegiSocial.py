...
URL_LEGISOCIAL = (
    "https://www.legisocial.fr/reperes-sociaux/plafond-securite-sociale-2025.html"
)
...
def get_plafonds_ss_legisocial() -> dict | None:
    ...
    try:
        ...
        if not table:
            ...
            table = soup.find("th", string=re.compile(r"Plafond annuel")).find_parent(
                "table"
            )
        ...
        if len(plafonds) < 5:
            raise ValueError(
                f"Tous les plafonds principaux n'ont pas été trouvés. Trouvés : {plafonds.keys()}"
            )
