...
def get_taux_csg_crds() -> dict | None:
    ...
    try:
        ...
        if len(taux_trouves) != 3:
            raise ValueError(
                f"Impossible de trouver les 3 taux CSG/CRDS. Trouvés : {taux_trouves.keys()}"
            )
        ...
        print(
            f"Calcul du taux non déductible : {taux_trouves['csg_imposable']} + {taux_trouves['crds']} = {taux_non_deductible:.4f}"
        )

        return {
            "deductible": taux_deductible,
            "non_deductible": round(taux_non_deductible, 5),
        }
