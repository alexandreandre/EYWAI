...
def get_taux_vieillesse_patronal() -> dict | None:
    ...
    try:
        ...
        if not employeur_section:
            raise ValueError(
                "La section 'Taux de cotisations employeur' est introuvable dans le HTML."
            )
        ...
        for row in table_rows:
            ...
            if header_cell and "Assurance vieillesse" in header_cell.get_text(
                ...
                print(
                    f" Taux vieillesse déplafonné (patronal) trouvé : {taux_deplafond * 100:.2f}%"
                )
                print(
                    f" Taux vieillesse plafonné (patronal) trouvé   : {taux_plafond * 100:.2f}%"
                )
