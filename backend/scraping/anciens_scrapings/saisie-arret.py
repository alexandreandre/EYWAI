...
def get_bareme_saisie() -> dict | None:
    ...
    try:
        ...
        bareme_header = soup.find(
            lambda tag: (
                tag.name in ["h3", "h2"]
                and "Barème" in tag.text
                and "saisissable" in tag.text
            )
        )
        if bareme_header:
            ...
            if bareme_table and bareme_table.find("tbody"):
                ...
                for row in rows:
                    ...
                    if len(cells) < 2:
                        continue  # On s'assure d'avoir au moins 2 colonnes
                    ...
                    if (
                        "€" not in tranche_texte
                        and "supérieur à" not in tranche_texte.lower()
                    ):
                        continue
                    ...
                    matches = re.findall(
                        r"([0-9\s\xa0,]+\.?[0-9]*)", tranche_texte.replace(".", "")
                    )
                    tranche_max = (
                        parse_valeur_numerique(matches[-1]) if matches else float("inf")
                    )

                    match_quotite = re.search(r"(\d+)/(\d+)", quotite_texte)
                    if "¼" in quotite_texte:
                        quotite_saisissable = 0.25
                    elif "1/3" in quotite_texte:
                        quotite_saisissable = round(1 / 3, 4)
                    elif "2/3" in quotite_texte:
                        quotite_saisissable = round(2 / 3, 4)
                    elif match_quotite:
                        quotite_saisissable = round(
                            int(match_quotite.group(1)) / int(match_quotite.group(2)), 4
                        )
                    else:
                        quotite_saisissable = 1.0  # Pour la ligne "supérieur à..."
                ...
                print(
                    f"  - Barème de saisie sur salaire trouvé ({len(bareme)} tranches)."
                )

        if len(donnees_saisie) < 2:
            raise ValueError(
                "Impossible de trouver toutes les données de saisie sur salaire."
            )
