    ...
    try:
        ...
        for row in table_rows:
            ...
            if header_cell and re.search(
                motif_recherche,
                header_cell.get_text(strip=True, separator=" "),
                flags=re.IGNORECASE,
            ):
                value_cell = row.find("td")
                if not value_cell:
                    raise ValueError(
                        "Ligne FNAL trouvée, mais cellule de valeur manquante."
                    )
                ...
                if not m:
                    raise ValueError(
                        f"Taux FNAL introuvable dans la cellule : '{value_text}'"
                    )
        ...
        raise ValueError(
            f"Ligne pour le taux FNAL ({effectif_log}) introuvable dans le tableau."
        )
