...
def _is_table_apec(tbl) -> bool:
    ...
    if (
        ("assiette tranche 1 + tranche 2" in head)
...
def get_all_taux_agirc_arrco() -> dict | None:
    ...
    try:
            ...
            if _is_table_retraite(table):
                ...
                for r_idx, row in enumerate(rows):
                    ...
                    if "tranche 1" in row_txt:
                        ...
                        target = (
                            rows[r_idx + 1]
                            if (r_idx + 1 < len(rows) and not _has_pct(row_txt))
                            else row
                        )
                        cells = target.find_all(["td", "th"])
                        # On cherche deux pourcentages: salar + patronal
                        pcts = [
                            parse_taux(c.get_text()) for c in cells if _has_pct(_txt(c))
                        ]
                        if len(pcts) >= 2:
        ...
        if len(taux) == expected_keys_count and all(
            v is not None for v in taux.values()
        ):
            print(
            ...
            raise ValueError(
                f"Certains taux sont manquants. Trouvés : {len(taux)}/{expected_keys_count}. Données: {taux}"
            )
