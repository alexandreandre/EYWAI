...
def get_taux_chomage_legisocial() -> float | None:
    ...
    try:
        ...
        if not chomage_header:
            raise ValueError(
                "Titre 'Cotisations chômage' introuvable. Le site bloque probablement Requests."
            )
        ...
        for row in table.find("tbody").find_all("tr"):
            ...
            if len(cells) > 4 and "Assurance chômage" in cells[0].get_text():
                ...
                if match:
                    ...
                    if taux is not None:
                        print(
                            f"Ligne trouvée : Taux de {taux * 100:.2f}% à partir du {start_date.strftime('%d/%m/%Y')}"
                        )
                        rates_by_date.append({"date_debut": start_date, "taux": taux})

        if not rates_by_date:
            raise ValueError(
                "Aucun taux d'assurance chômage avec date n'a pu être extrait."
            )
        ...
        raise ValueError(
            "Impossible de déterminer le taux applicable à la date d'aujourd'hui."
        )
