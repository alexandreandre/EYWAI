...
def get_valeurs_avantages() -> dict | None:
    ...
    try:
        ...
        if repas_header:
            ...
            if repas_table:
                ...
                for row in rows:
                    ...
                    if header_cell and re.search(
                        r"1\s+repas", header_cell.get_text(strip=True)
                    ):
        ...
        if tr_header:
            ...
            if tr_table:
                header_cell = tr_table.find(
                    lambda tag: (
                        tag.name == "th" and "Exonération maximale" in tag.get_text()
                    )
                )
                if header_cell:
                    ...
                    if row and row.find("td"):
                        ...
                        print(
                            f"  - Exonération max. titre-restaurant trouvée : {valeur_tr} €"
                        )
        ...
        if logement_header:
            ...
            if logement_table and logement_table.find("tbody"):
                ...
                if bareme:
                    ...
                    print(
                        f"  - Barème logement complet trouvé ({len(bareme)} tranches)."
                    )

        if len(valeurs_trouvees) < 3:
            raise ValueError(
                f"Impossible de trouver toutes les valeurs. Trouvées : {list(valeurs_trouvees.keys())}"
            )
