...
def get_taux_alloc_legisocial() -> dict | None:
    ...
    try:
        ...
        table_title = soup.find(
            lambda tag: (
                tag.name in ["h2", "h3"] and "Quels sont les taux" in tag.get_text()
            )
        )
        if not table_title:
            raise ValueError(
                "Titre de la table principale des cotisations introuvable."
            )
                ...
                if "allocations familiales" in libelle:
                    ...
                    if taux is not None:
                        ...
                        if "≤" in libelle or "<" in libelle:
                            print(
                                f"Taux Allocations Familiales (réduit) trouvé : {taux * 100:.2f}%"
                            )
                            taux_trouves["reduit"] = taux
                        elif ">" in libelle:
                            print(
                                f"Taux Allocations Familiales (plein) trouvé : {taux * 100:.2f}%"
                            )
