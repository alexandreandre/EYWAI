...
def get_bareme_saisie_legisocial() -> dict | None:
    ...
    try:
        ...
        sbi_header = soup.find(
            lambda tag: (
                tag.name in ["h2", "h3"]
                and "quotité insaisissable" in tag.get_text().lower()
            )
        )
        if sbi_header:
            ...
            if sbi_list:
                for item in sbi_list.find_all("li"):
                    if "depuis le 1er avril" in item.get_text():
                        ...
                        if sbi_match:
                            donnees_saisie["sbi"] = parse_valeur_numerique(
                                sbi_match.group(1)
                            )
                            print(
                                f"  - Solde Bancaire Insaisissable (SBI) trouvé : {donnees_saisie['sbi']} €"
                            )
                            break
        ...
        monthly_table_title_cell = soup.find(
            lambda tag: (
                tag.name == "td"
                and "barème mensuel" in tag.get_text(strip=True).lower()
            )
        )
        if not monthly_table_title_cell:
            raise ValueError(
                "Impossible de trouver la cellule de titre 'Barème mensuel'."
            )

        table = monthly_table_title_cell.find_parent("table")
        if not table:
            raise ValueError(
                "Impossible de trouver la table parente du barème mensuel."
            )
        ...
        for row in table.find("tbody").find_all("tr"):
            ...
            if "majoration pour personne à charge" in row_text:
                ...
                if majoration_match:
                    donnees_saisie["majoration"] = parse_valeur_numerique(
                        majoration_match.group(1)
                    )
                    print(
                        f"  - Majoration par personne à charge trouvée : {donnees_saisie['majoration']} €"
                    )
            else:
                cells = row.find_all("td")
                if len(cells) == 2 and (
                    "€" in cells[0].get_text()
                    or "totalité" in cells[1].get_text().lower()
                ):
        ...
        if len(donnees_saisie) < 3:
            raise ValueError(
                f"Impossible de trouver toutes les données. Trouvées: {donnees_saisie.keys()}"
            )
