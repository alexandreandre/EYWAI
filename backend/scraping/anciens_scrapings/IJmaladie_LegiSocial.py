...
def get_all_plafonds_ij_legisocial() -> dict | None:
    ...
    try:
        ...
        for header in soup.find_all("h3"):
            ...
            # --- IJ Maternité/Paternité: prendre la 1re valeur uniquement ---
                ...
                if row:
                    ...
                    if len(cells) >= 2:
                        ...
                        print(
                            f"  - Plafond IJ Maternité/Paternité trouvé : {valeur} € (1re valeur retenue)"
                        )
            ...
            # --- IJ AT/MP: prendre les 1res occurrences uniquement ---
                ...
                for row in tbody.find_all("tr"):
                    ...
                    valeur_cell = next(
                        (cell for cell in cells if "€" in cell.get_text()), None
                    )
                    if not valeur_cell:
                        continue
                    ...
                    if "jusqu'au 28" in libelle and "at_mp" not in plafonds:
                        ...
                        print(
                            f"  - Plafond IJ AT/MP trouvé : {valeur} € (1re valeur retenue)"
                        )
                    elif "partir du 29" in libelle and "at_mp_majoree" not in plafonds:
                        plafonds["at_mp_majoree"] = valeur
                        print(
                            f"  - Plafond IJ AT/MP majorée trouvé : {valeur} € (1re valeur retenue)"
                        )
        ...
        if set(plafonds.keys()) != {
            ...
            raise ValueError(
                f"Toutes les indemnités n'ont pas été trouvées. Trouvées : {plafonds}"
            )
