...
def scrape_repas_data() -> dict | None:
    ...
    if (
        "repas_valeur_forfaitaire" in data
        and "titre_restaurant_exoneration_max_patronale" in data
    ):
...
def scrape_logement_data() -> list | None:
    ...
    if len(rows) != 3:
        raise ValueError(
            "La structure de la table logement n'est pas celle attendue (3 lignes)."
        )
        ...
        bareme.append(
            {
                ...
                "valeur_1_piece": parse_valeur_numerique(
                    valeurs_1_piece_cells[i].get_text()
                ),
                "valeur_par_piece": parse_valeur_numerique(
                    valeurs_par_piece_cells[i].get_text()
                ),
            }
        )
...
def get_and_update_avantages():
    ...
    try:
        ...
        final_data = {
            ...
            "titre_restaurant_exoneration_max_patronale": repas_data[
        ...
        avantages = config["PARAMETRES_ENTREPRISE"].setdefault(
            "avantages_en_nature", {}
        )
