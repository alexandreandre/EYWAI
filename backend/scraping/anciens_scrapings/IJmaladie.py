    ...
    try:
        ...
        for row in all_rows:
            ...
            if len(cells) == 2:
                ...
                if "29e jour" in libelle:
                    ...
                    plafonds["maternite_paternite"] = parse_valeur_numerique(
                        valeur_text
                    )
        ...
        if len(plafonds) != 4:
            with open("debug_ameli.html", "w", encoding="utf-8") as f:
                f.write(page_html)
            raise ValueError(
                f"Toutes les indemnités n'ont pas été trouvées. Trouvées : {len(plafonds)} sur 4. Fichier 'debug_ameli.html' créé."
            )
