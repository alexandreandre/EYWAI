...
def get_taux_csg_crds_legisocial() -> dict | None:
    ...
    try:
        ...
        if not csg_header:
            raise ValueError(
                "Titre de la section 'COTISATIONS CSG et CRDS' introuvable."
            )

        table = csg_header.find_next("table")
        if not table:
            raise ValueError(
                "Table des cotisations CSG/CRDS introuvable après le titre."
            )
        ...
        for row in table.find("tbody").find_all("tr"):
            ...
            if len(cells) > salarie_col_index:
                ...
                if "CSG déductible" in libelle and "deductible" not in taux_bruts:
                    ...
                    "CSG non déductible" in libelle
                    and "non_deductible_csg" not in taux_bruts
                ):
                    taux_bruts["non_deductible_csg"] = parse_taux(taux_text)
                elif (
                    "CRDS non déductible" in libelle
                    and "non_deductible_crds" not in taux_bruts
                ):
        ...
        if len(taux_bruts) != 3 or None in taux_bruts.values():
            raise ValueError(
                f"Impossible de trouver les 3 taux CSG/CRDS. Trouvés : {taux_bruts}"
            )

        print(f"Taux bruts extraits : {taux_bruts}")

        taux_final_deductible = taux_bruts["deductible"]
        taux_final_non_deductible = (
            taux_bruts["non_deductible_csg"] + taux_bruts["non_deductible_crds"]
        )

        return {
            "deductible": taux_final_deductible,
            "non_deductible": round(taux_final_non_deductible, 5),
        }
