...
def get_taux_maladie_legisocial() -> dict | None:
    ...
    try:
                ...
                if "maladie" in libelle and "alsace-moselle" not in libelle:
                    ...
                    if taux is not None:
                        ...
                        if "≤" in libelle or "<" in libelle:
                            print(
                                f"Taux Maladie Patronal (réduit) trouvé : {taux * 100:.2f}%"
                            )
                            taux_trouves["reduit"] = taux
                        elif ">" in libelle:
                            print(
                                f"Taux Maladie Patronal (plein) trouvé : {taux * 100:.2f}%"
                            )
        ...
        if "reduit" in taux_trouves and "plein" in taux_trouves:
            ...
            raise ValueError(
                "Impossible de trouver les deux taux maladie (réduit et plein)."
            )
