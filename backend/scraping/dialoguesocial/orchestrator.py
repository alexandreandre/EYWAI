...
ITEM_ID_TO_PATCH = "dialogue_social"
...
SCRIPTS_TO_RUN: List[Tuple[str, str]] = [
    ("dialoguesocial.py", os.path.join(os.path.dirname(__file__), "dialoguesocial.py")),
    ("dialoguesocial_AI.py", os.path.join(os.path.dirname(__file__), "dialoguesocial_AI.py")),
    # Note: Pas de scraper LegiSocial pour cette contribution
]
...
def core_signature(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Normalise le payload en une signature comparable pour le dialogue social."""
...
def equal_core(a: Dict[str, Any], b: Dict[str, Any], tol: float = 1e-9) -> bool:
    """Compare deux signatures 'dialogue_social'."""
...
def apply_patch_in_memory(
    ...
    """
    Met à jour le bloc 'cotisations' complet avec le patch 'dialogue_social'.
    """
