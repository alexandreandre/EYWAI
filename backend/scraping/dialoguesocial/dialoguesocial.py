...
def scrape_dialogue_social_rate() -> float | None:
    ...
    try:
        ...
        for row in soup.find_all("tr"):
            ...
            if (
                header_cell
                and "Contribution au dialogue social"
        ...
        if not target_row:
            raise ValueError(
                "Impossible de trouver la ligne <tr> contenant l'en-tête 'Contribution au dialogue social'."
            )
...
def main():
    ...
    if rate_data is None:
        print(
            "ERREUR CRITIQUE: Le scraping du taux de dialogue social a échoué.",
            file=sys.stderr,
        )
        sys.exit(1)
