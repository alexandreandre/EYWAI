# scripts/MMIDpatronal/orchestrator.py (ADAPTED FOR SUPABASE + DEBUG)
...
def core_signature(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Normalise le payload en une signature comparable pour MMID."""
    ...
    plein_norm = parse_rate(plein, script_name)
    reduit_norm = parse_rate(reduit, script_name)
...
def equal_core(a: Dict[str, Any], b: Dict[str, Any], tol: float = 1e-9) -> bool:
    """Compare les taux essentiels de la signature."""
...
def apply_patch_in_memory(
    ...
    for i, item in enumerate(cotisations_list):
        if isinstance(item, dict) and item.get("id") == ITEM_ID_TO_PATCH:
            ...
            # Préserve les clés non gérées par ce script (ex: salarial_Alsace_Moselle)
