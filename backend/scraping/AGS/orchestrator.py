...
def debug_mismatch(payloads: List[Dict[str, Any]], sigs: List[Dict[str, Any]]) -> None:
    ...
    line = " | ".join(
        f"{p.get('__script', '?')}={s.get('valeurs', {}).get('patronal', 'N/A')}"
...
def init_supabase_client() -> Client:
    ...
    if not supabase_url or not supabase_key:
        ...
        logging.critical(
            "Assurez-vous que .env contient SUPABASE_SERVICE_KEY (la clé 'service_role') et non la clé 'anon'."
        )
...
def fetch_active_config(supabase: Client, config_key: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if response is None:
            logging.error(
                "La requête execute() a retourné 'None'. Problème de connexion, de permission (406) ou d'API."
            )
            return None
...
def _build_run_env(supabase: Client) -> Dict[str, str]:
    ...
    try:
        row = (
            supabase.table("payroll_config")
            .select("config_data")
            .eq("config_key", "parametres_entreprise")
            .eq("is_active", True)
            .limit(1)
            .execute()
        )
        if row.data and len(row.data) > 0:
            ...
            is_ett = bool(
                data.get("PARAMETRES_ENTREPRISE", {})
...
def main() -> None:
    ...
    try:
        ...
        new_config_data_blob = apply_patch_in_memory(
            current_row["config_data"] if current_row else None, ags_patch_data
        )
