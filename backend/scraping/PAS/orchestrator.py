...
def equal_core(sig_a: Dict, sig_b: Dict) -> Tuple[bool, Optional[str]]:
    ...
    if len(baremes_a) != len(baremes_b):
        return (
            False,
            f"Le nombre de zones de barème diffère ({len(baremes_a)} vs {len(baremes_b)})",
        )
    ...
    for zone_a, zone_b in zip(baremes_a, baremes_b):
        if zone_a.get("zone") != zone_b.get("zone"):
            return (
                False,
                f"Zones de barème non alignées: {zone_a.get('zone')} vs {zone_b.get('zone')}",
            )
        ...
        if len(tranches_a) != len(tranches_b):
            return (
                False,
                f"Mismatch dans '{zone_name}': le nombre de tranches diffère ({len(tranches_a)} vs {len(tranches_b)})",
            )
        ...
        for i, (tranche_a, tranche_b) in enumerate(zip(tranches_a, tranches_b)):
            if not compare_floats(tranche_a.get("plafond"), tranche_b.get("plafond")):
                return (
                    False,
                    f"Mismatch dans '{zone_name}[{i}].plafond': {tranche_a.get('plafond')} != {tranche_b.get('plafond')}",
                )
            if not compare_floats(tranche_a.get("taux"), tranche_b.get("taux")):
                return (
                    False,
                    f"Mismatch dans '{zone_name}[{i}].taux': {tranche_a.get('taux')} != {tranche_b.get('taux')}",
                )
...
def main() -> None:
    ...
    try:
        ...
        # sigs[ref_idx] contient {"baremes": [...]}
        final_data_to_store = sigs[ref_idx]
