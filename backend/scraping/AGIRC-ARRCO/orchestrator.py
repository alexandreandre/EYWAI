...
def core_signature(payload: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    if payload.get("id") != "agirc_arrco_bundle" or payload.get("type") != "cotisation_bundle":
        logging.error(f"Payload inattendu reçu de {payload.get('__script')}. Bundle AGIRC-ARRCO requis.")
    ...
    for it in items:
        ...
        try:
            ...
            logging.warning(f"Valeur non numérique (sal='{sal}', pat='{pat}') reçue pour l'ID '{_id}' depuis {payload.get('__script')}.")
            sal_norm, pat_norm = None, None

        # ✅ CORRECTION: Force la base correcte
        correct_base = BASE_MAPPING.get(_id)
        if not correct_base:
            logging.warning(f"ID '{_id}' non trouvé dans BASE_MAPPING. Utilisation de la base scrapée: '{it.get('base')}'")
    ...
    if missing:
        logging.error(f"Items manquants dans le bundle de {payload.get('__script')}: {missing}")
...
def debug_compare(per_source: List[Tuple[str, Dict[str, Dict[str, Any]]]]) -> bool:
    ...
    for cid in ITEMS_ID_TO_PATCH:
        ...
        for label, core_bundle in per_source:
            ...
            line_parts.append(f"{label}[S:{fmt(sal)}|P:{fmt(pat)}|B:{base}]") # Ajout de la base
...
def apply_patch_in_memory(
    ...
    """
    logging.info(f"Application du bundle de patchs ({', '.join(patches_dict.keys())}) en mémoire...")
    ...
    for patch_id, patch_core in patches_dict.items():
        if patch_id not in ITEMS_ID_TO_PATCH:
            logging.warning(f"ID '{patch_id}' ignoré (non listé dans ITEMS_ID_TO_PATCH).")
            continue
...
def update_config_in_supabase(
    ...
    if current_config_data == new_config_data:
        logging.info(f"Les données '{comment}' sont inchangées. Mise à jour de 'last_checked_at'.")
        try:
    ...
    # --- SCÉNARIO 2 : DIFFÉRENT ---
        ...
        logging.warning(f"Différence détectée pour '{comment}'. Création de la version {current_version + 1}...")
