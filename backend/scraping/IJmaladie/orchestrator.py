...
def core_signature(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for k in FIELDS:
        ...
        try:
            ...
            logging.warning(
                f"Valeur non numérique '{v}' pour '{k}' reçue de {payload.get('__script')}."
            )
            core_vals[k] = None
...
def debug_mismatch(payloads: List[Dict[str, Any]], sigs: List[Dict[str, Any]]) -> None:
    ...
    for k in FIELDS:
        line = " | ".join(
            f"{p.get('__script', '?')}={s['valeurs'].get(k)}"
...
def main() -> None:
    ...
    try:
        ...
        final_data_to_store = sigs[ref_idx]["valeurs"]
