...
def core_signature(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logging.warning(
            f"Structure 'sections' non trouvée dans {payload.get('__script')}. Tentative avec 'valeurs.salarial_Alsace_Moselle'."
        )
    ...
    if alsace_raw is not None:
        try:
            ...
            logging.warning(
                f"Valeur Alsace Moselle non numérique '{alsace_raw}' reçue de {payload.get('__script')}. Traitée comme None."
            )
    ...
    return {
        ...
        "valeurs": {"salarial_Alsace_Moselle": alsace},
    }
...
def equal_core(a: Dict[str, Any], b: Dict[str, Any], tol: float = 1e-9) -> bool:
    ...
    return compare_floats(
        a_vals.get("salarial_Alsace_Moselle"),
        b_vals.get("salarial_Alsace_Moselle"),
        tol,
    )
...
def debug_mismatch(payloads: List[Dict[str, Any]], sigs: List[Dict[str, Any]]) -> None:
    ...
    logging.warning(
        f"DISCORDANCE entre les sources pour {ITEM_ID_TO_PATCH} (Salarial Alsace):"
    )
    line = " | ".join(
...
def debug_success(payloads: List[Dict[str, Any]], sigs: List[Dict[str, Any]]) -> None:
    ...
    logging.info(
        f"Concordance du taux {ITEM_ID_TO_PATCH} (Salarial Alsace) trouvée => {line}"
    )
...
def apply_patch_in_memory(
    ...
    """
    logging.info(
        f"Application du patch '{ITEM_ID_TO_PATCH}' (Salarial Alsace) en mémoire..."
    )
    ...
    for i, item in enumerate(cotisations_list):
        if isinstance(item, dict) and item.get("id") == ITEM_ID_TO_PATCH:
            ...
            found = True
            logging.info(
                f"Item '{ITEM_ID_TO_PATCH}' trouvé et mis à jour (Salarial Alsace)."
            )
            break
...
def update_config_in_supabase(
    ...
    if current_config_data == new_config_data:
        logging.info(
            f"Les données '{ITEM_ID_TO_PATCH}' (Salarial Alsace) sont inchangées. Mise à jour de 'last_checked_at'."
        )
        try:
    ...
    # --- SCÉNARIO 2 : DIFFÉRENT ---
        ...
        logging.warning(
            f"Différence détectée pour '{ITEM_ID_TO_PATCH}' (Salarial Alsace). Création de la version {current_version + 1}..."
        )
