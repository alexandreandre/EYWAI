#!/usr/bin/env python3
"""Source IA — assurance vieillesse patronale (recherche web)."""
...
def main() -> None:
    ...
    data = extract_with_web_search(
        task_prompt=(
            f"Extrais les deux taux PATRONAUX de l'assurance vieillesse URSSAF "
            f"applicables en {cy} : plafonné (jusqu'au plafond SS) et déplafonné "
            f"(sur la totalité du salaire). Renvoie plafond_percent et deplafond_percent en %."
        ),
        json_schema=SCHEMA,
        schema_name="vieillesse_patronal",
        include_domains=OFFICIAL,
    )
    if not data or data.get("plafond_percent") is None or data.get("deplafond_percent") is None:
        print("ERREUR CRITIQUE: extraction IA vieillesse patronale échouée.", file=sys.stderr)
        sys.exit(1)
    ...
    payload = build_standard_payload(
        item_id="assurance_vieillesse_patronal",
        item_type="taux_cotisation",
        libelle="Taux de cotisation patronale - Assurance Vieillesse",
        sections_or_valeurs=sections,
        generator="vieillessepatronal/vieillessepatronal_AI.py",
        source_url=URL,
        source_label="URSSAF vieillesse patronale (IA web)",
    )
    emit_ai_payload_or_exit(payload, "assurance_vieillesse_patronal")
