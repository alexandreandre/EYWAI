...
def get_taux_vieillesse_patronal() -> dict | None:
    ...
    try:
        ...
        if not employeur_section:
            raise ValueError(
                "La section 'Taux de cotisations employeur' est introuvable."
            )
        ...
        for row in table_rows:
            ...
            if header_cell and "Assurance vieillesse" in header_cell.get_text(
                strip=True
            ):
                value_cell = row.find("td")
                if not value_cell:
                    raise ValueError(
                        "Ligne 'Assurance vieillesse' trouvée, mais cellule de valeur manquante."
                    )
                ...
                if not m:
                    raise ValueError(
                        f"Taux d'assurance vieillesse patronale introuvables dans la cellule : '{value_text}'"
                    )
                ...
                print(
                    f"  - Taux vieillesse déplafonné (patronal) trouvé : {taux_deplafonne * 100:.2f}%",
                    file=sys.stderr,
                )
                print(
                    f"  - Taux vieillesse plafonné (patronal) trouvé   : {taux_plafonne * 100:.2f}%",
                    file=sys.stderr,
                )
        ...
        raise ValueError(
            "Ligne 'Assurance vieillesse' introuvable dans le tableau employeur."
        )
...
def main():
    ...
    if not taux_data:
        print(
            "ERREUR CRITIQUE: Le scraping des taux de vieillesse patronaux a échoué.",
            file=sys.stderr,
        )
        sys.exit(1)
