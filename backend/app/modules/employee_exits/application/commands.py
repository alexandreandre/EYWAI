...
def create_employee_exit(
    ...
    notice_start = None
    notice_end = None
    if exit_data.get("notice_period_days", 0) > 0 and not exit_data.get(
    ...
    record = build_exit_record(
        ...
        exit_request_date=exit_data["exit_request_date"].isoformat()
        if hasattr(exit_data["exit_request_date"], "isoformat")
        else exit_data["exit_request_date"],
        last_working_day=exit_data["last_working_day"].isoformat()
        if hasattr(exit_data["last_working_day"], "isoformat")
        else exit_data["last_working_day"],
...
def update_exit_status(
    ...
    if (
        ...
        and current_status == "rupture_validee"
        and new_status == "rupture_effective"
    ):
        if exit_data.get("validation_date"):
            from datetime import datetime as dt

            vd = dt.fromisoformat(
                exit_data["validation_date"].replace("Z", "+00:00")
                if isinstance(exit_data["validation_date"], str)
                else exit_data["validation_date"]
            )
...
def generate_exit_document(
    ...
    if document_type not in GENERATABLE_DOCUMENT_TYPES:
        raise EmployeeExitApplicationError(
            400, f"Type de document non générable automatiquement: {document_type}"
        )
    sb = supabase_client or supabase
    exit_repo = EmployeeExitRepository(sb)
    doc_repo = ExitDocumentRepository(sb)
    ...
    if document_type == "certificat_travail":
        ...
        pdf_bytes = generator.generate_solde_tout_compte(
            employee_data, company_data, exit_data, indemnities, sb
        )
    else:
...
def delete_exit_document(
    exit_id: str, document_id: str, company_id: str, supabase_client: Any = None
) -> None:
...
def add_checklist_item(
    ...
    item_record = {
        ...
        "due_date": item_data.get("due_date").isoformat()
        if item_data.get("due_date") and hasattr(item_data.get("due_date"), "isoformat")
        else item_data.get("due_date"),
...
def mark_checklist_item_complete(
    ...
    if item_update.get("due_date") is not None:
        update_data["due_date"] = (
            item_update["due_date"].isoformat()
            if hasattr(item_update["due_date"], "isoformat")
            else item_update["due_date"]
        )
    if update_data:
...
def delete_checklist_item(
    exit_id: str, item_id: str, company_id: str, supabase_client: Any = None
) -> None:
    ...
    if item.get("is_required"):
        raise EmployeeExitApplicationError(
            400, "Impossible de supprimer un item obligatoire"
        )
...
def edit_exit_document(
    ...
    if document_type == "certificat_travail":
        ...
        pdf_bytes = generator.generate_solde_tout_compte(
            gen_employee, gen_company, gen_exit, indemnities, sb
        )
    else:
        raise EmployeeExitApplicationError(
    ...
    for doc in documents:
        ...
        try:
            try:
                ...
                total_failed += 1
                continue
            existing = get_employee_document_by_source_exit_document(
                str(employee_id), doc["id"], sb
            )
            if existing and not force_update:
                doc_status["status"] = "already_published"
                doc_status["employee_document_id"] = UUID(existing["id"])
                doc_status["published_at"] = (
                    datetime.fromisoformat(
                        existing["published_at"].replace("Z", "+00:00")
                    )
            ...
            try:
                ...
                raise Exception(
                    f"Impossible de télécharger le fichier source: {str(e)}"
                )
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            dest_path = (
                f"employees/{employee_id}/documents/{doc['document_type']}_{ts}.pdf"
            )
            storage.upload(
                dest_path, file_bytes, doc.get("mime_type", "application/pdf")
            )
