...
# périmètre.
_TOOL_HANDLERS: dict[
    ToolName, Callable[[str, dict[str, Any], str], dict[str, Any]]
] = {
    ToolName.EMPLOYEE_COUNT: lambda cid, args, uid: secure_queries.count_employees(cid, args),
    ToolName.EMPLOYEE_SEARCH: lambda cid, args, uid: secure_queries.search_employees(cid, args),
    ToolName.PAYROLL_SUMMARY: lambda cid, args, uid: secure_queries.payroll_summary(cid, args),
    ToolName.ABSENCE_SUMMARY: lambda cid, args, uid: secure_queries.absence_summary(cid, args),
    ToolName.PLANNING_SUMMARY: lambda cid, args, uid: secure_queries.planning_summary(cid, args),
    ToolName.HR_INDICATORS: lambda cid, args, uid: secure_queries.hr_indicators(cid, args),
    ToolName.ABSENCES_EN_COURS: lambda cid, args, uid: secure_queries.absences_en_cours(
        cid, args, uid
    ),
    ToolName.ECHEANCES_RH: lambda cid, args, uid: secure_queries.echeances_rh(
        cid, args, uid
    ),
    ToolName.EMPLOYEE_DETAIL: lambda cid, args, uid: secure_queries.employee_detail(
        cid, args, uid
    ),
}


def execute_tool(call: ToolCall, company_id: str, user_id: str = "") -> dict[str, Any]:
    ...
    return handler(company_id, call.arguments, user_id)
