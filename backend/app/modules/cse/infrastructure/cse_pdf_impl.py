...
def generate_convocation_pdf(meeting_data: Dict[str, Any]) -> bytes:
    ...
    story.append(
        Paragraph(f"<b>Titre:</b> {meeting_data.get('title', 'N/A')}", styles["Normal"])
    )
    ...
    if meeting_date:
        ...
        story.append(
            Paragraph(f"<b>Date:</b> {date_obj.strftime('%d/%m/%Y')}", styles["Normal"])
        )
    ...
    story.append(
        Paragraph(
            f"<b>Type:</b> {type_labels.get(meeting_type, meeting_type)}",
            styles["Normal"],
        )
    )
...
def generate_minutes_pdf(
    ...
    story.append(
        Paragraph(
            f"<b>Réunion:</b> {meeting_data.get('title', 'N/A')}", styles["Normal"]
        )
    )
...
def generate_election_calendar_pdf(
    ...
    story.append(
        Paragraph(
            f"<b>Cycle:</b> {cycle_data.get('cycle_name', 'N/A')}", styles["Normal"]
        )
    )
    ...
    if mandate_end:
        date_obj = (
            datetime.fromisoformat(mandate_end)
            if isinstance(mandate_end, str)
            else mandate_end
        )
        story.append(
            Paragraph(
                f"<b>Fin de mandat:</b> {date_obj.strftime('%d/%m/%Y')}",
                styles["Normal"],
            )
        )
    ...
    if election_date:
        date_obj = (
            datetime.fromisoformat(election_date)
            if isinstance(election_date, str)
            else election_date
        )
        story.append(
            Paragraph(
                f"<b>Date des élections:</b> {date_obj.strftime('%d/%m/%Y')}",
                styles["Normal"],
            )
        )
