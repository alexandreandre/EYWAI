...
def _is_elected_member(company_id: str, employee_id: str) -> bool:
    ...
    response = supabase.table('cse_elected_members') \
        .select('id') \
...
def get_elected_members(
    ...
    query = supabase.table('cse_elected_members').select(
        """
        id,
        employee_id,
        role,
    ...
    for member in members:
        ...
        end_date = datetime.fromisoformat(member['end_date']).date() if isinstance(member['end_date'], str) else member['end_date']
        days_remaining = (end_date - date.today()).days if end_date >= date.today() else None
        
        result.append(ElectedMemberListItem(
            id=member['id'],
            ...
            start_date=datetime.fromisoformat(member['start_date']).date() if isinstance(member['start_date'], str) else member['start_date'],
            end_date=end_date,
...
def create_elected_member(
    ...
    if not response.data:
        raise HTTPException(status_code=500, detail="Erreur lors de la création du mandat")
...
def update_elected_member(
    ...
    existing = supabase.table('cse_elected_members') \
...
def get_elected_member_by_id(member_id: str) -> ElectedMemberRead:
    ...
    response = supabase.table('cse_elected_members').select(
        """
        *,
        employees!inner(
            id,
    ...
    return ElectedMemberRead(
        ...
        updated_at=datetime.fromisoformat(member['updated_at']) if isinstance(member['updated_at'], str) else member['updated_at'],
        first_name=employee.get('first_name'),
        last_name=employee.get('last_name'),
        job_title=employee.get('job_title')
    )
...
def get_meetings(
    ...
    if participant_id:
        ...
        query = supabase.table('cse_meeting_participants').select(
            """
            meeting_id,
            cse_meetings!inner(
                id,
        ...
        query = supabase.table('cse_meetings').select(
            """
            id,
            title,
            meeting_date,
    ...
    for meeting in meetings:
        ...
        result.append(MeetingListItem(
            id=meeting['id'],
            title=meeting['title'],
            meeting_date=datetime.fromisoformat(meeting['meeting_date']).date() if isinstance(meeting['meeting_date'], str) else meeting['meeting_date'],
            meeting_time=_parse_time(meeting.get('meeting_time')),
            meeting_type=meeting['meeting_type'],
            status=meeting['status'],
            participant_count=participant_count,
            created_at=datetime.fromisoformat(meeting['created_at']) if isinstance(meeting['created_at'], str) else meeting['created_at']
        ))
...
def get_meeting_by_id(
    ...
    response = supabase.table('cse_meetings').select(
        """
        *,
        ...
        """
    ).eq('id', meeting_id).eq('company_id', company_id).execute()
    ...
    for participant in meeting.get('cse_meeting_participants', []):
        ...
        participants.append(MeetingParticipantRead(
            ...
            invited_at=datetime.fromisoformat(participant['invited_at']) if participant.get('invited_at') and isinstance(participant['invited_at'], str) else participant.get('invited_at'),
            confirmed_at=datetime.fromisoformat(participant['confirmed_at']) if participant.get('confirmed_at') and isinstance(participant['confirmed_at'], str) else participant.get('confirmed_at'),
    ...
    return MeetingRead(
        ...
        created_at=datetime.fromisoformat(meeting['created_at']) if isinstance(meeting['created_at'], str) else meeting['created_at'],
        updated_at=datetime.fromisoformat(meeting['updated_at']) if isinstance(meeting['updated_at'], str) else meeting['updated_at'],
...
def update_meeting(
    ...
    existing = supabase.table('cse_meetings') \
        .select('id') \
        .eq('id', meeting_id) \
        .eq('company_id', company_id) \
        .execute()
    ...
    response = supabase.table('cse_meetings') \
        .update(update_data) \
        .eq('id', meeting_id) \
        .execute()
...
def add_participants(
    ...
    # Vérifier que la réunion existe
    meeting_response = supabase.table('cse_meetings').select('id, company_id').eq('id', meeting_id).execute()
    ...
    response = supabase.table('cse_meeting_participants').insert(participants_data).execute()
    
    if not response.data:
        raise HTTPException(status_code=500, detail="Erreur lors de l'ajout des participants")
...
def get_meeting_participants(meeting_id: str) -> List[MeetingParticipantRead]:
    ...
    response = supabase.table('cse_meeting_participants').select(
        """
        *,
        employees!inner(
            id,
...
def start_recording(
    ...
    consent_data = [
        ...
        for c in consents if c.get('consent_given')
    ]
    
    # Vérifier si un enregistrement existe déjà
    existing_response = supabase.table('cse_meeting_recordings') \
        .select('id') \
        .eq('meeting_id', meeting_id) \
        .execute()
    ...
    if existing_response.data:
        ...
        response = supabase.table('cse_meeting_recordings') \
            .update(update_data) \
            .eq('meeting_id', meeting_id) \
            .execute()
    else:
        ...
        response = supabase.table('cse_meeting_recordings').insert(insert_data).execute()
    
    if not response.data:
        raise HTTPException(status_code=500, detail="Erreur lors du démarrage de l'enregistrement")
...
def stop_recording(meeting_id: str, company_id: str) -> RecordingStatusRead:
    ...
    # Vérifier que la réunion existe
    meeting_response = supabase.table('cse_meetings').select('id').eq('id', meeting_id).eq('company_id', company_id).execute()
    if not meeting_response.data:
        raise HTTPException(status_code=404, detail="Réunion non trouvée")
...
def get_recording_status(meeting_id: str) -> RecordingStatusRead:
    ...
    response = supabase.table('cse_meeting_recordings').select('*').eq('meeting_id', meeting_id).execute()
    ...
    return RecordingStatusRead(
        ...
        recording_started_at=datetime.fromisoformat(recording['recording_started_at']) if recording.get('recording_started_at') and isinstance(recording['recording_started_at'], str) else recording.get('recording_started_at'),
        recording_ended_at=datetime.fromisoformat(recording['recording_ended_at']) if recording.get('recording_ended_at') and isinstance(recording['recording_ended_at'], str) else recording.get('recording_ended_at'),
...
def get_delegation_quota(
    ...
    employee_response = supabase.table('employees').select('collective_agreement_id').eq('id', employee_id).execute()
    if not employee_response.data:
        return None
    ...
    if not collective_agreement_id:
        ...
        company_response = supabase.table('company_collective_agreements').select('collective_agreement_id').eq('company_id', company_id).limit(1).execute()
        if company_response.data:
            collective_agreement_id = company_response.data[0].get('collective_agreement_id')
...
def get_delegation_hours(
    ...
    query = supabase.table('cse_delegation_hours').select(
        """
        *,
        employees!inner(
            id,
    ...
    for hour in response.data or []:
        ...
        hours.append(DelegationHourRead(
            id=hour['id'],
            company_id=hour['company_id'],
            employee_id=hour['employee_id'],
            date=datetime.fromisoformat(hour['date']).date() if isinstance(hour['date'], str) else hour['date'],
            duration_hours=float(hour['duration_hours']),
            reason=hour['reason'],
            meeting_id=hour.get('meeting_id'),
            created_by=hour.get('created_by'),
            created_at=datetime.fromisoformat(hour['created_at']) if isinstance(hour['created_at'], str) else hour['created_at'],
            first_name=employee.get('first_name'),
            last_name=employee.get('last_name')
        ))
...
def create_delegation_hour(
    ...
    # Vérifier que l'employé existe
    employee_response = supabase.table('employees').select('id').eq('id', employee_id).eq('company_id', company_id).execute()
    if not employee_response.data:
        raise HTTPException(status_code=404, detail="Employé non trouvé")
...
def get_delegation_summary(
    ...
    for member in elected_members:
        ...
        hours = get_delegation_hours(company_id, member.employee_id, period_start, period_end)
        consumed_hours = sum(h.duration_hours for h in hours)
        
        summaries.append(DelegationSummary(
            employee_id=member.employee_id,
...
def get_bdes_documents(
    ...
    query = supabase.table('cse_bdes_documents').select(
        """
        *,
    ...
    for doc in response.data or []:
        ...
        documents.append(BDESDocumentRead(
            id=doc['id'],
            ...
            year=doc.get('year'),
            published_at=datetime.fromisoformat(doc['published_at']) if doc.get('published_at') and isinstance(doc['published_at'], str) else doc.get('published_at'),
...
def get_bdes_document_by_id(document_id: str, company_id: str) -> BDESDocumentRead:
    ...
    response = supabase.table('cse_bdes_documents').select(
        """
        *,
        published_by_profile:profiles!cse_bdes_documents_published_by_fkey(
            id,
...
def _create_default_timeline(cycle_id: str, mandate_end_date: date) -> None:
    ...
    steps = [
        {'name': 'Déclenchement des élections', 'order': 1, 'due_date': mandate_end_date - timedelta(days=180)},
        {'name': 'Dépôt des listes', 'order': 2, 'due_date': mandate_end_date - timedelta(days=90)},
        {'name': 'Campagne électorale', 'order': 3, 'due_date': mandate_end_date - timedelta(days=30)},
        {'name': 'Élections', 'order': 4, 'due_date': mandate_end_date},
        {'name': 'Proclamation des résultats', 'order': 5, 'due_date': mandate_end_date + timedelta(days=7)},
    ]
...
def get_election_cycle_by_id(cycle_id: str, company_id: str) -> ElectionCycleRead:
    ...
    response = supabase.table('cse_election_cycles').select(
        """
        *,
        ...
        """
    ).eq('id', cycle_id).eq('company_id', company_id).execute()
    ...
    for step in cycle.get('cse_election_timeline', []):
        timeline.append(ElectionTimelineStepRead(
            id=step['id'],
            election_cycle_id=step['election_cycle_id'],
            step_name=step['step_name'],
            step_order=step['step_order'],
            due_date=datetime.fromisoformat(step['due_date']).date() if isinstance(step['due_date'], str) else step['due_date'],
            completed_at=datetime.fromisoformat(step['completed_at']) if step.get('completed_at') and isinstance(step['completed_at'], str) else step.get('completed_at'),
            status=step['status'],
            notes=step.get('notes'),
            created_at=datetime.fromisoformat(step['created_at']) if isinstance(step['created_at'], str) else step['created_at'],
            updated_at=datetime.fromisoformat(step['updated_at']) if isinstance(step['updated_at'], str) else step['updated_at']
        ))
    
    mandate_end = datetime.fromisoformat(cycle['mandate_end_date']).date() if isinstance(cycle['mandate_end_date'], str) else cycle['mandate_end_date']
    ...
    return ElectionCycleRead(
        ...
        election_date=datetime.fromisoformat(cycle['election_date']).date() if cycle.get('election_date') and isinstance(cycle['election_date'], str) else cycle.get('election_date'),
        status=cycle['status'],
        results_pdf_path=cycle.get('results_pdf_path'),
        minutes_pdf_path=cycle.get('minutes_pdf_path'),
        notes=cycle.get('notes'),
        created_at=datetime.fromisoformat(cycle['created_at']) if isinstance(cycle['created_at'], str) else cycle['created_at'],
        updated_at=datetime.fromisoformat(cycle['updated_at']) if isinstance(cycle['updated_at'], str) else cycle['updated_at'],
        timeline=timeline,
...
def get_election_cycles(company_id: str) -> List[ElectionCycleRead]:
    ...
    response = supabase.table('cse_election_cycles').select('id').eq('company_id', company_id).order('mandate_end_date', desc=True).execute()
