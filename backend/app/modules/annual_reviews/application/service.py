...
from app.modules.annual_reviews.infrastructure.repository import annual_review_repository
from app.modules.annual_reviews.infrastructure.providers import get_annual_review_pdf_generator
...
def generate_annual_review_pdf(
    ...
    employee_name = (
        f"{employee_data.get('first_name', '')}_{employee_data.get('last_name', '')}"
        .strip()
        .replace(" ", "_")
    )
