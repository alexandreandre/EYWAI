...
def get_employee_mutuelle_choices(
    ...
    if not emp_resp.data:
        raise MutuelleTypeApplicationError(404, "Employé non trouvé.")
    ...
    *,
    ...
    if not allow_rh_override and not psc.get("mutuelle_employee_self_service", False):
        raise MutuelleTypeApplicationError(
            403,
            "Le choix mutuelle par le salarié est désactivé pour cette entreprise.",
        )
    emp_resp = (
        supabase.table("employees")
        .select("id, statut")
    ...
    if mutuelle is None:
        raise MutuelleTypeApplicationError(404, "Formule mutuelle non trouvée.")
    if not is_mutuelle_eligible_for_employee(
        ...
        raise MutuelleTypeApplicationError(
            400,
            "Cette formule mutuelle n'est pas compatible avec votre statut.",
        )
