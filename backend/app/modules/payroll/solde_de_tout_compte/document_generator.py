...
# Import case modules
from app.modules.payroll.solde_de_tout_compte.cases.demission import generate_demission_solde
from app.modules.payroll.solde_de_tout_compte.cases.rupture_conventionnelle import generate_rupture_conventionnelle_solde
from app.modules.payroll.solde_de_tout_compte.cases.licenciement import generate_licenciement_solde
from app.modules.payroll.solde_de_tout_compte.cases.retraite import generate_retraite_solde
from app.modules.payroll.solde_de_tout_compte.cases.fin_periode_essai import generate_fin_periode_essai_solde
from app.modules.payroll.solde_de_tout_compte.cases.generic import generate_generic_solde
...
class EmployeeExitDocumentGenerator:
    ...
    def generate_certificat_travail(
        ...
        company_name = company_data.get('name') or company_data.get('raison_sociale') or "l'entreprise"
        texte_certif = f"""
        ...
        story.append(Spacer(1, 0.3*cm))
        story.append(Paragraph("Signature et cachet de l'entreprise :", self.styles['Signature']))
    ...
    def _get_salary_prorata(self, employee_data: Dict[str, Any], exit_data: Dict[str, Any]) -> Dict[str, Any]:
        """Calcule le prorata du salaire du dernier mois (méthode wrapper pour compatibilité)"""
        from app.modules.payroll.solde_de_tout_compte.common.socle_commun import get_salary_prorata
    ...
    def generate_solde_tout_compte(
        ...
        if exit_type == 'demission':
            return generate_demission_solde(
                self.styles, employee_data, company_data, exit_data, indemnities, supabase_client
            )
    ...
    def generate_attestation_pole_emploi(
        ...
        # Section employeur
        story.append(Paragraph("<b>INFORMATIONS EMPLOYEUR</b>", self.styles['Important']))
        story.append(Spacer(1, 0.3*cm))

        data_employeur = [
            ['Raison sociale :', company_data.get('name', company_data.get('raison_sociale', ''))],
            ['SIRET :', company_data.get('siret', '')],
            ['Adresse :', company_data.get('address', '')]
        ]

        table_employeur = Table(data_employeur, colWidths=[5*cm, 11*cm])
        table_employeur.setStyle(TableStyle([
            ('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'),
            ('FONTSIZE', (0, 0), (-1, -1), 10),
            ('LEFTPADDING', (0, 0), (-1, -1), 0),
        ...
        data_salarie = [
            ['Nom et prénom :', nom_complet],
            ['Date de naissance :', self._format_date(employee_data.get('date_naissance', ''))],
            ...
            ['Date de fin de contrat :', self._format_date(exit_data.get('last_working_day', ''))]
        ]

        table_salarie = Table(data_salarie, colWidths=[5*cm, 11*cm])
        table_salarie.setStyle(TableStyle([
            ('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'),
            ('FONTSIZE', (0, 0), (-1, -1), 10),
            ('LEFTPADDING', (0, 0), (-1, -1), 0),
            ('BOTTOMPADDING', (0, 0), (-1, -1), 6),
        ...
        story.append(Paragraph("<b>MOTIF DE FIN DE CONTRAT</b>", self.styles['Important']))
        story.append(Spacer(1, 0.3*cm))
        ...
        if exit_data.get('exit_reason'):
            story.append(Paragraph(
                f"Détails : {exit_data['exit_reason']}",
