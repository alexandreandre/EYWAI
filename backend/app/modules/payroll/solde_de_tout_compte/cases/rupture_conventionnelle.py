...
def generate_rupture_conventionnelle_solde(
    ...
    story.append(
        Paragraph(
            "<b>2. INDEMNITÉ SPÉCIFIQUE DE RUPTURE CONVENTIONNELLE</b>",
            styles["Important"],
        )
    )
    ...
    montant_rupture = pdf_helpers.safe_float(
        indemnite_rupture.get("montant_negocie", 0)
    )
    montant_minimum = pdf_helpers.safe_float(
        indemnite_rupture.get("montant_minimum", 0)
    )
    ...
    montant_conges = socle_commun.build_conges_section(
        story, styles, indemnities, section_number=3
    )
