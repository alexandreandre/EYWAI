...
def generate_fin_periode_essai_solde(
    ...
    if notice_compensation_due is None:
        ...
        if probation_ended_by == "employer" and notice_period_respected is False:
            ...
            notice_compensation_due = (
                False  # En général, pas d'indemnité si salarié rompt
            )
        else:
    ...
    story.append(
        Paragraph(
            "<b>3. INDEMNITÉ COMPENSATRICE DE DÉLAI DE PRÉVENANCE</b>",
            styles["Important"],
        )
    )
    story.append(Spacer(1, 0.3 * cm))
    ...
    data_prevenance = [
        ...
        [
            ...
            detail_prevenance,
            pdf_helpers.format_currency(montant_prevenance)
            if montant_prevenance > 0
            else "",
        ],
    ]

    table_prevenance = Table(data_prevenance, colWidths=[6 * cm, 7 * cm, 3 * cm])
    table_prevenance.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#e5e7eb")),
                ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
    ...
    montant_conges = socle_commun.build_conges_section(
        story, styles, indemnities, section_number=4
    )
    ...
    pdf_helpers.build_legal_mentions(
        story, styles, specific_mention=mention_periode_essai
    )
