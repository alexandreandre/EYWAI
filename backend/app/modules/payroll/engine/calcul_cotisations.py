    ...
    if mutuelle_spec.get('adhesion'):
        ...
        if mutuelle_type_ids:
            try:
                ...
                print(f"WARN: Impossible de charger les mutuelles pour CSG: {e}", file=sys.stderr)
...
def calculer_cotisations(
    ...
    print(f"  -> Adhésion prévoyance détectée dans le contrat: {adhesion_prevoyance}", file=sys.stderr)
    ...
    if adhesion_prevoyance and contexte.statut_salarie == 'Cadre':
        ...
        print(f"  -> Lignes spécifiques trouvées pour le cadre: {len(lignes_specifiques)}", file=sys.stderr)
        for ligne in lignes_specifiques:
            ...
            if ligne_calculee:
                bulletin_cotisations.append(ligne_calculee)
                print(f"    -> Ligne calculée (Cadre): {ligne_calculee}", file=sys.stderr)
                ...
                if taux_fs and ligne_calculee.get('montant_patronal', 0) > 0:
                    ...
                    if ligne_fs:
                        ...
                        print(f"      -> Ligne Forfait Social ajoutée: {ligne_fs}", file=sys.stderr)
        ...
        if coti_data:
            print(f"  -> Règle 'prevoyance_non_cadre' trouvée dans les barèmes: {json.dumps(coti_data)}", file=sys.stderr)
            assiette = assiettes.get(coti_data.get('base', 'brut_plafonne'), 0.0)
            print(f"  -> Assiette de calcul utilisée: {assiette:.2f} €", file=sys.stderr)
            ligne_calculee = _calculer_une_ligne(
            ...
            if ligne_calculee:
                bulletin_cotisations.append(ligne_calculee)
                print(f"  -> ✅ Ligne calculée (Non-Cadre) et ajoutée: {ligne_calculee}", file=sys.stderr)
            else:
                print("  -> ❌ Ligne non-cadre non ajoutée (calcul a retourné None ou 0).", file=sys.stderr)
        else:
            print("  -> ❌ Règle 'prevoyance_non_cadre' INTROUVABLE dans les barèmes (cotisations.json).", file=sys.stderr)
    else:
        print("  -> ❌ Aucune branche de calcul de prévoyance n'a été exécutée.", file=sys.stderr)
