...
def analyser_jours_forfait_du_mois(
    ...
    """
    print(f"INFO: Analyse des jours forfait pour {employee_name} - {mois:02d}/{annee}...", file=sys.stderr)
    ...
    if date_debut_periode and date_fin_periode:
        print(f"DEBUG: Période de paie : du {date_debut_periode.strftime('%d/%m/%Y')} au {date_fin_periode.strftime('%d/%m/%Y')}", file=sys.stderr)
        prevu_par_mois = {}
        reel_par_mois = {}
        for j in prevu_data:
            m = j.get('mois', '?')
    ...
    for cle_semaine, data in semaines.items():
        ...
        for jour_prevu in data["jours_non_travailles"]:
            jour_travaille_reel = any(
                ...
                (j.get('heures_faites') or 0) == 1  # Jour travaillé = 1
                for j in data['reel']
            )
            ...
            if heures_faites != 1:
                ...
                if type_jour == 'travail':
                    ...
                    if ev_absence['mois'] in [1, 2]:
                        print(f"DEBUG: Absence détectée : {ev_absence['jour']:02d}/{ev_absence['mois']:02d}/{ev_absence['annee']}", file=sys.stderr)
        ...
        if date_debut_periode and date_fin_periode:
            ...
            if not (date_debut_periode <= ev_date <= date_fin_periode):
                if "absence" in ev.get('type', ''):
                    print(f"DEBUG: Événement d'absence exclu (hors période) : {ev_date.strftime('%d/%m/%Y')} (événement: {ev.get('type')})", file=sys.stderr)
                continue
        else:
    ...
    if absences_par_mois:
        print(f"DEBUG: Absences détectées par mois: {absences_par_mois}", file=sys.stderr)
    
    print(f"DEBUG: evenements_agreges={len(evenements_agreges)} événements", file=sys.stderr)
    return sorted(evenements_agreges, key=lambda x: (x.get('annee', annee), x.get('mois', mois), x.get('jour', 0)))
...
if __name__ == "__main__":
    ...
    try:
        ...
        chemin_calendrier_prevu = chemin_employe / 'calendriers' / f'{args.mois:02d}.json'
        ...
        prevu_data = json.loads(chemin_calendrier_prevu.read_text(encoding='utf-8')).get('calendrier_prevu', [])
        reel_data = []
        if chemin_horaires_reels.exists():
            reel_data = json.loads(chemin_horaires_reels.read_text(encoding='utf-8')).get('calendrier_reel', [])
