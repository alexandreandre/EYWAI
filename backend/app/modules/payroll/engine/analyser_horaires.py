...
def analyser_horaires_du_mois(
    ...
    fichiers = [(annee_prec, mois_prec), (annee, mois), (annee_suiv, mois_suiv)]
    ...
    for a, m in fichiers:
        ...
        if cp.exists():
            for j in json.loads(cp.read_text(encoding="utf-8")).get(
                "calendrier_prevu", []
            ):
                j = {**j, "annee": a, "mois": m}
                prevu_data.append(j)
        if cr.exists():
            for j in json.loads(cr.read_text(encoding="utf-8")).get(
                "calendrier_reel", []
            ):
                j = {**j, "annee": a, "mois": m}
                reel_data.append(j)
    ...
    from app.modules.payroll.application.analyzer import (
        analyser_horaires_du_mois as _analyser_in_memory,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Analyse les horaires prévus et réels pour générer un fichier d'événements de paie."
    )
    ...
    try:
        ...
        if not chemin_contrat.exists():
            raise FileNotFoundError(
                f"Le fichier contrat.json est introuvable pour {args.nom_employe}"
            )

        contrat_data = json.loads(chemin_contrat.read_text(encoding="utf-8"))
        duree_hebdo = (
            contrat_data.get("contrat", {})
            .get("temps_travail", {})
            .get("duree_hebdomadaire")
        )
        if not duree_hebdo:
            raise ValueError(
                f"La 'duree_hebdomadaire' n'est pas définie dans le contrat.json de {args.nom_employe}"
            )

        evenements = analyser_horaires_du_mois(
            chemin_employe, args.annee, args.mois, duree_hebdo
        )
