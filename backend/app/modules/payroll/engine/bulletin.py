...
def creer_bulletin_final(
    ...
    bulletin = {
        ...
        "synthese_net": {
            ...
            "acompte_verse": resultats_nets.get('acompte_verse', 0.0),  # Montant des avances déduites
        },
...
def creer_bulletin_sortie(
    ...
    bulletin_base = creer_bulletin_final(
        contexte, salaire_brut, details_brut, lignes_cotisations,
        resultats_nets, primes_non_soumises, annee, mois
    )
    ...
    if indemnites_sortie.get('indemnite_licenciement'):
        ...
        if ind_lic.get('montant', 0) > 0:
            lignes_indemnites_exonerees.append({
                ...
                'description': ind_lic.get('description', 'Article L1234-9 du Code du travail'),
    ...
    total_indemnites_soumises = sum(ligne['gain'] for ligne in lignes_indemnites_soumises)
    total_indemnites_exonerees = sum(ligne['montant'] for ligne in lignes_indemnites_exonerees)
    ...
    net_a_payer_final = resultats_nets.get('net_a_payer', 0) + total_indemnites_soumises + total_indemnites_exonerees
    ...
    bulletin_base['indemnites_sortie'] = {
        ...
        'total_general': round(total_indemnites_soumises + total_indemnites_exonerees, 2)
    }

    # Mettre à jour les totaux du bulletin
    bulletin_base['salaire_brut_avec_indemnites_soumises'] = round(brut_total_avec_indemnites, 2)
    ...
    bulletin_base['pied_de_page']['cout_total_employeur'] = round(
        ...
        total_primes_non_soumises + total_indemnites_exonerees, 2
    )

    print(f"INFO: Bulletin de sortie assemblé - Indemnités totales: {round(total_indemnites_soumises + total_indemnites_exonerees, 2)} €", file=sys.stderr)
