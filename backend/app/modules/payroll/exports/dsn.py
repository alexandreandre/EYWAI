...
def get_dsn_employees_data(
    ...
    """
    Récupère les données des salariés pour la DSN

    Returns:
        tuple: (liste des salariés avec leurs bulletins, totaux)
    """
    ...
    for employee in employees:
        ...
        net_imposable = float(payslip_data.get('synthese_net', {}).get('net_imposable', 0) if isinstance(payslip_data.get('synthese_net'), dict) else 0)
        pas = float(payslip_data.get('synthese_net', {}).get('impot_preleve_a_la_source', 0) if isinstance(payslip_data.get('synthese_net'), dict) else 0)
        ...
        cotisations_salariales = sum(float(c.get('montant_salarial', 0) or 0) for c in cotisations_list if isinstance(c, dict))
        cotisations_patronales = sum(float(c.get('montant_patronal', 0) or 0) for c in cotisations_list if isinstance(c, dict))
...
def check_dsn_data(
    ...
    """
    Effectue tous les contrôles DSN

    Returns:
        tuple: (anomalies_bloquantes, avertissements)
    """
    anomalies = []
    warnings = []
    ...
    if not address or not isinstance(address, dict):
        ...
        if not address.get('rue') or not address.get('ville') or not address.get('code_postal'):
            warnings.append("Adresse établissement incomplète (rue, ville ou code postal manquant)")
    ...
    for emp_data in employees_data:
        ...
        if not adresse or not isinstance(adresse, dict):
            ...
            if not adresse.get('rue') or not adresse.get('ville') or not adresse.get('code_postal'):
                warnings.append(f"Salarié {employee_name} : adresse incomplète")
        ...
        if isinstance(specificites, dict):
            ...
            if contract_type in ['CDI', 'CDD'] and not mutuelle.get('adhesion'):
                warnings.append(f"Salarié {employee_name} : mutuelle absente alors que contrat actif")

            if contract_type in ['CDI', 'CDD'] and not prevoyance.get('adhesion'):
                warnings.append(f"Salarié {employee_name} : prévoyance absente alors que contrat actif")

        hire_date = employee.get('hire_date')
        if hire_date:
            try:
                year, month = map(int, period.split("-"))
                hire_dt = datetime.strptime(hire_date, '%Y-%m-%d') if isinstance(hire_date, str) else hire_date
                period_dt = datetime(year, month, 1)
                if hire_dt.month == period_dt.month and hire_dt.year == period_dt.year:
                    warnings.append(f"Salarié {employee_name} : entré en cours de mois ({hire_date})")
            except Exception:
                pass
...
def preview_dsn(
    ...
    """
    Prévise une DSN sans générer de fichier

    Returns:
        dict: Résumé avec totaux, anomalies, avertissements
    """
    company_data = get_company_data(company_id)
    employees_data, totals = get_dsn_employees_data(company_id, period, employee_ids)

    anomalies, warnings = check_dsn_data(company_id, period, employees_data, company_data)
    ...
    for emp_data in employees_data:
        ...
        for coti in cotisations_detail:
            if isinstance(coti, dict):
                ...
                elif 'RETRAITE' in libelle.upper() or 'AGIRC' in libelle.upper() or 'ARRCO' in libelle.upper():
    ...
    for emp_data in employees_data:
        ...
        for coti in cotisations_detail:
            if isinstance(coti, dict):
                ...
                organismes_summary[organisme]['nombre_salaries'].add(emp_data['employee']['id'])
                organismes_summary[organisme]['total_cotisations_salariales'] += float(coti.get('montant_salarial', 0) or 0)
                organismes_summary[organisme]['total_cotisations_patronales'] += float(coti.get('montant_patronal', 0) or 0)
...
def generate_dsn_xml(
    ...
    for emp_data in employees_data:
        ...
        ET.SubElement(remuneration, "NetImposable").text = str(emp_data.get('net_imposable', 0))
        ...
        for coti in emp_data.get('cotisations_detail', []):
            if isinstance(coti, dict):
                ...
                ET.SubElement(cotisation, "TauxSalarial").text = str(coti.get('taux_salarial', 0) or 0)
                ET.SubElement(cotisation, "TauxPatronal").text = str(coti.get('taux_patronal', 0) or 0)
                ET.SubElement(cotisation, "MontantSalarial").text = str(coti.get('montant_salarial', 0) or 0)
                ET.SubElement(cotisation, "MontantPatronal").text = str(coti.get('montant_patronal', 0) or 0)
