# app/modules/payroll/exports/journal_paie.py
# Migré depuis services/exports/journal_paie.py. Export Journal de paie.
...
def get_journal_paie_data(
    ...
    """
    Récupère les données du journal de paie pour une période donnée

    Returns:
        tuple: (données, totaux)
    """
...
def generate_journal_paie_export(
    ...
    """Génère l'export du journal de paie"""
...
def preview_journal_paie(
    ...
    """Prévise l'export du journal de paie sans générer de fichier"""
    data, totals = get_journal_paie_data(company_id, period, employee_ids)
    ...
    if totals['employees_count'] > 0:
        expected_net = totals['total_brut'] - totals['total_cotisations_salariales'] - totals.get('total_pas', 0)
        diff = abs(totals['total_net_a_payer'] - expected_net)

        if diff > 1.0:
            warnings.append(f"Écart de {diff:.2f}€ entre le net calculé et le net à payer")
