...
def _ensure_onboarding_view(
    ...
    if not employee_queries.employee_belongs_to_company(employee_id, company_id):
...
def get_my_onboarding(current_user: User = Depends(get_current_user)):
    ...
    data = onboarding_queries.get_or_create_checklist(str(eid), company_id)
...
def get_onboarding(employee_id: str, current_user: User = Depends(get_current_user)):
    ...
    data = onboarding_queries.get_or_create_checklist(employee_id, company_id)
    return OnboardingChecklistOut(**data)
...
def complete_onboarding_task(
    ...
    _ensure_onboarding_view(current_user, company_id, employee_id)
    cl = onboarding_queries.get_checklist(employee_id, company_id)
    if not cl:
    ...
    ok = onboarding_commands.complete_task(
...
def uncomplete_onboarding_task(
    ...
    if not any(str(t["id"]) == str(task_id) for t in cl.get("tasks", [])):
        raise HTTPException(status_code=404, detail="Tâche introuvable.")
    ok = onboarding_commands.uncomplete_task(task_id, checklist_id, company_id)
    if not ok:
