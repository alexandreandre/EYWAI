...
REPRISE_ABSENCE_TYPES = ["arret_maternite", "arret_at", "arret_maladie_pro", "arret_maladie"]
...
def _get_employee_collective_agreement_idcc(supabase: Any, employee_id: str) -> Optional[str]:
    emp = (
...
def _get_absence_return_dates_for_reprise(supabase: Any, employee_id: str) -> List[Dict[str, Any]]:
    today = date.today()
    req = (
...
def _existing_obligations(supabase: Any, company_id: str, employee_id: str) -> List[Dict[str, Any]]:
    req = (
...
def _dedupe_key(
    visit_type: str, trigger_type: str, due_date: date, request_date: Optional[date] = None
):
...
def compute_obligations_for_employee(company_id: str, employee_id: str) -> List[Dict[str, Any]]:
    """
    ...
    for ob_id, payload in to_update:
        supabase.table("medical_follow_up_obligations").update(payload).eq("id", ob_id).execute()
