...
class UserCompanyAccessData(BaseModel):
    ...
    role_template_id: Optional[UUID] = Field(
        None,
        description="ID du template de rôle à appliquer"
    )
    permission_ids: List[UUID] = Field(
        default_factory=list,
        description="Liste des IDs de permissions additionnelles"
    )
...
class UserCreateWithPermissions(BaseModel):
    ...
    job_title: Optional[str] = Field(None, max_length=100, description="Titre du poste global")
...
class UserUpdateWithPermissions(BaseModel):
    ...
    base_role: Optional[str] = Field(None, pattern="^(admin|rh|collaborateur_rh|collaborateur|custom)$")
