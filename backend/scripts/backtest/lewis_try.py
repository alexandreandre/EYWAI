"""Applique un patch employé ponctuel pour Lewis, revert-safe (garde si tier-S baisse).

Copie stricte de mbc_try.py (même mécanisme snapshot/apply/compare/revert), seule
différence : COMPANY = "Lewis". Voir mbc_try.py pour la doc complète du format --patch.

Usage:
  .venv/bin/python -m scripts.backtest.lewis_try MATRICULE --patch '<json>'
  .venv/bin/python -m scripts.backtest.lewis_try MATRICULE --patch-file /path.json
