...
- Aucune migration ne doit réutiliser un horodatage existant dans `supabase/migrations/` — l'horodatage est la clé primaire de la CLI Supabase. Les migrations s'appliquent automatiquement en production depuis le 31/07/2026. Sur l'environnement de test, le déploiement n'en jouait aucune jusqu'au commit `a4e6ede1` (02/08/2026), qui ajoute à `deploy-test-env.yml` une étape optionnelle où l'opérateur **nomme** la migration à appliquer. Les deux migrations de ce plan doivent donc être passées explicitement au déploiement du test, une par une : `20260803090000_monthly_inputs_quantity_kind.sql` puis `20260803091000_bonus_types_situation_repas.sql`. Ne pas tenter un `supabase db push` sur le test : la resynchronisation ne copie que le schéma `public`, donc son historique `supabase_migrations` n'est pas fiable.
