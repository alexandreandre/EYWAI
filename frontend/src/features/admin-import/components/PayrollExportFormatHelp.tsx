...
const EXPECTED_COLUMNS = [
  ...
  { label: 'Organisation', cols: 'Service, Handicapé (Oui → code 01 RQTH)' },
  { label: 'Titre séjour', cols: 'N° carte, dates obtention / expiration' },
];

export function PayrollExportFormatHelp({
  className,
  modMoiTeams = false,
}: {
  className?: string;
  /** Affiche le mapping Service → équipes MOD/MOI lorsque activé pour l'entreprise. */
  modMoiTeams?: boolean;
}) {
  const organisationCols = modMoiTeams
    ? 'Service (MOD/MOI/CAD → équipes MOD ou MOI), Handicapé (Oui → code 01 RQTH)'
    : 'Service (informatif), Handicapé (Oui → code 01 RQTH)';

  const columns = EXPECTED_COLUMNS.map((row) =>
    row.label === 'Organisation' ? { ...row, cols: organisationCols } : row,
  );
      ...
      <CollapsibleContent className="border-t px-3 pb-3 pt-2 text-sm text-muted-foreground space-y-2">
        ...
        <p className="text-xs">
          ...
          {!modMoiTeams ? (
            <>
              {' '}
              Le mapping vers les équipes MOD/MOI n&apos;est pas activé pour cette entreprise — la
              colonne Service reste visible à titre informatif.
            </>
          ) : null}
        </p>
      </CollapsibleContent>
